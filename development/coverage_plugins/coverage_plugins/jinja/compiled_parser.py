...
def _blank_only_yield_lines_by_generated_line(
    ...
    source_lines_by_generated_line = dict(debug_map)
    first_blank_source_line_by_if_line = _first_blank_source_line_before_next_block(source, source_static_lines)
    ...
    for node in ast.walk(tree):
        ...
        if source_line is None or source_line not in first_blank_source_line_by_if_line or not node.body:
            continue

        first_statement = next((statement for statement in node.body if not isinstance(statement, ast.Pass)), None)
        if (
            ...
            blank_only_yield_lines[first_statement.value.lineno] = first_blank_source_line_by_if_line[source_line]
...
def _first_blank_source_line_before_next_block(source: str, source_static_lines: tuple[tuple[int, str], ...]) -> dict[int, int]:
    """Return the first blank static source line immediately inside an ``if`` body."""
    ...
    blank_source_lines_by_if_line: dict[int, int] = {}
    for block_line, block_name in block_lines:
        if block_name not in {"if", "elif"}:
            continue
        ...
        if blank_line is not None and (next_block_line is None or blank_line < next_block_line):
            blank_source_lines_by_if_line[block_line] = blank_line

    return blank_source_lines_by_if_line
