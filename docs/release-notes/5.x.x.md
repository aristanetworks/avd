...
### Python 3.13.10 and 3.13.11 Incompatibility

AVD 5.7.x is incompatible with Python 3.13.10 and 3.13.11 due to a critical bug in CPython's split table
dictionary implementation. These Python versions cause crashes when running AVD due to incorrect handling
of dictionary key membership checks in objects using `__slots__`.

**Root Cause:** CPython bugs [gh-142218](https://github.com/python/cpython/issues/142218) and
[gh-143189](https://github.com/python/cpython/issues/143189) introduced a regression in Python 3.13.10
that was only fully fixed in Python 3.13.12.

**Symptoms:** Running AVD with affected Python versions may result in:

- `TypeError: argument of type 'NoneType' is not iterable`
- Crashes during structured config generation
- Unexpected behavior in eos_designs role
...
#### Removal of Ansible Plugins in AVD 6.0.0

- `arista.avd.configlet_build_config`
- `arista.avd.inventory_to_container`
- `arista.avd.eos_validate_state_report`
- `arista.avd.eos_validate_state_runner`

#### Deprecations on eos_designs data models

The following data model keys have been deprecated from eos_designs in AVD 5.7.3 and will be removed in AVD 6.0.0.
...
!!! note
    When upgrading to AVD 6.0, refer to the porting-guide for all required changes.

#### Deprecations on eos_cli_config_gen data models

The following data model keys have been deprecated from eos_cli_config_gen in AVD 5.7.3 and will be removed in AVD 6.0.0.

| Deprecated key | New key in AVD 6.0 | Notes for AVD 6.0 |
| -------------- | ------------------ | ----------------- |
| `avd_data_validation_mode` | - | Validation is always required. |
| `dhcp_servers[].subnets` | `dhcp_servers[].ipv4_subnets` or `dhcp_servers[].ipv6_subnets` | Improved input data-model for IPv4 and IPv6 subnets. |
| `ethernet_interfaces[].peer` | `ethernet_interfaces[].metadata.peer` | Only used for validation and documentation purposes. |
| `ethernet_interfaces[].peer_interface` | `ethernet_interfaces[].metadata.peer_interface` | Only used for validation and documentation purposes. |
| `ethernet_interfaces[].peer_key` | `ethernet_interfaces[].metadata.peer_key` | Only used for validation and documentation purposes. |
| `ethernet_interfaces[].peer_type` | `ethernet_interfaces[].metadata.peer_type` | Only used for validation and documentation purposes. |
| `ethernet_interfaces[].port_porfile` | `ethernet_interfaces[].metadata.port_porfile` | Only used for validation and documentation purposes. |
| `ethernet_interfaces[].validate_lldp` | `ethernet_interfaces[].metadata.validate_lldp` | Only used for validation and documentation purposes. |
| `ethernet_interfaces[].validate_state` | `ethernet_interfaces[].metadata.validate_state` | Only used for validation and documentation purposes. |
| `ip_ftp_client_source_interfaces` | `ip_ftp_client.source_interface` or `ip_ftp_client.vrfs[].source_interface` | Improved input data model for CLI ordering. |
| `ip_http_client_source_interfaces` | `ip_http_client.source_interface` or `ip_http_client.vrfs[].source_interface` | Improved input data model for CLI ordering. |
| `ip_name_servers` | ip_name_server.servers | Improved input data model for CLI ordering. |
| `ip_ssh_client_source_interfaces` | `ip_ssh_client.source_interface` or `ip_ssh_client.vrfs[].source_interface` | Improved input data model for CLI ordering. |
| `ip_telnet_client_source_interfaces` | `ip_telnet_client.source_interface` or `ip_telnet_client.vrfs[].source_interface` | Improved input data model for CLI ordering. |
| `ip_tftp_client_source_interfaces` | `ip_tftp_client.source_interface` or `ip_tftp_client.vrfs[].source_interface` | Improved input data model for CLI ordering. |
| `is_deployed` | `metadata.is_deployed` | Only used for validation and documentation purposes. |
| `ntp.servers[].vrf` | `ntp.vrf` | EOS supports one VRF for all NTP servers. |
| `port_channel_interfaces[].peer` | `port_channel_interfaces[].metadata.peer` | Only used for validation and documentation purposes. |
| `port_channel_interfaces[].peer_interface` | `port_channel_interfaces[].metadata.peer_interface` | Only used for validation and documentation purposes. |
| `port_channel_interfaces[].peer_key` | `port_channel_interfaces[].metadata.peer_key` | Only used for validation and documentation purposes. |
| `port_channel_interfaces[].peer_type` | `port_channel_interfaces[].metadata.peer_type` | Only used for validation and documentation purposes. |
| `port_channel_interfaces[].validate_lldp` | `port_channel_interfaces[].metadata.validate_lldp` | Only used for validation and documentation purposes. |
| `port_channel_interfaces[].validate_state` | `port_channel_interfaces[].metadata.validate_state` | Only used for validation and documentation purposes. |
| `radius_server.hosts` | `radius_server.servers` or `radius_server.vrfs[].servers` | Improved input data model for CLI ordering. |
| `router_bgp.neighbor_interfaces[].peer` | `router_bgp.neighbor_interfaces[].metadata.peer` | Only used for validation and documentation purposes. |
| `router_bgp.neighbors[].peer` | `router_bgp.neighbors[].metadata.peer` | Only used for validation and documentation purposes. |
| `router_bgp.peer_groups[].type` | `router_bgp.peer_groups[].metadata.type` | Only used for validation and documentation purposes. |
| `router_bgp.vlan_aware_bundles[].description` | `router_bgp.vlan_aware_bundles[].metadata.description` | Only used for validation and documentation purposes. |
| `router_bgp.vlan_aware_bundles.tenant` | `router_bgp.vlan_aware_bundles.metadata.tenants` | Only used for validation and documentation purposes. |
| `router_bgp.vlans.tenant` | `router_bgp.vlans.metadata.tenant` | Only used for validation and documentation purposes. |
| `serial_number` | `metadata.serial_number` | Only used for validation and documentation purposes. |
...
| `vlan_interfaces[].tags` | `vlan_interfaces[].metadata.tags` | Only used for validation and documentation purposes. |
| `vlan_interfaces[].tenant` | `vlan_interfaces[].metadata.tenant` | Only used for validation and documentation purposes. |
| `vlan_interfaces[].type` | `vlan_interfaces[].metadata.type` | Only used for validation and documentation purposes. |
| `vlans[].tenant` | `vlans[].metadata.tenants` | Only used for validation and documentation purposes. |
| `vrfs[].tenant` | `vrfs[].metadata.tenants` | Only used for validation and documentation purposes. |
...
For more details, see the `arista.avd.cv_deploy` [role documentation](../../ansible_collections/arista/avd/roles/cv_deploy/README.md).
