...
!!! note
    This is a **naming change only** for the schemas. The Ansible role names remain unchanged:

    - The `arista.avd.eos_designs` role continues to use the same name.
    - The `arista.avd.eos_cli_config_gen` role continues to use the same name.
