...
- Python requirements:
  - PyAVD now does **not** require the `cvprac`, the `treelib` not the `jsonschema` requirements
...
- Ansible collection requirements:
  - `arista.avd` Ansible collection does **not** depend anymore on `arista.cvp` and `ansible.utils` collections.
