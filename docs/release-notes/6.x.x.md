...
- Python requirements:
  - PyAVD now requires the `pyavd-utils` library. This library is maintained alongside `pyavd` and is co-versioned. It provides Python bindings to Rust code. This should not have any impact
    if you are installing `pyavd` with `pip`; everything will install transparently. If you are a developer contributing to AVD, you will need to install `cargo`. Refer to the [contribution guide](../contribution/overview.md).
  ...
  - PyAVD now supports ansible-core from **2.16.0** to **2.19.x**.
    The latest version can be installed with `pip install "ansible-core<2.20" --upgrade`.
    !!! Note
