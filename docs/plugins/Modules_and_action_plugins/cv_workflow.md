...
| <samp>&nbsp;&nbsp;&nbsp;&nbsp;use_system_certs</samp> | bool | optional | False | - | Use system certificate and honor overrides with `SSL_CERT_FILE` and `SSL_CERT_DIR`.<br>Prefer the OS trust store over the bundled `certifi` Python package<br>(certifi is only used as a fallback when the OS provides no usable trust store). |
