...
| `underlay_mutlticast` | `underlay_mutlticast_pim_sm` or `<node_type_key>.defaults.underlay_multicast.pim_sm.enabled` or `<node_type_key>.node_groups[].underlay_multicast.pim_sm.enabled` or `<node_type_key>.node_groups[].nodes[].underlay_multicast.pim_sm.enabled` or `<node_type_key>.node_groups[].underlay_multicast.pim_sm.enabled`or `<node_type_key>.node_groups[].underlay_multicast.pim_sm.enabled` or `<node_type_key>.nodes[].underlay_multicast.pim_sm.enabled` |
...
In AVD 5.x, to enable PIM SM for a `core_interface` or `l3_edge` link, it was required to have the global `underlay_mutlicast: true` and under the P2P link (or its profile)
`include_in_underlay_protocol: true` and `underlay_mutlicast: true`. If any condition was not set, PIM SM was not enabled.

As of AVD 6.0.0, the global `underlay_mutlicast` is removed and replaced with node setting `underlay_multicast.pim_sm.enabled` or global `underlay_mutlicast_pim_sm`.
Additionally `core_interfaces.p2p_links[].underlay_multicast` and `l3_edge.p2p_links[].underlay_multicast` are removed and replaced with respectively
`core_interfaces.p2p_links[].multicast_pim_sm` and `l3_edge.p2p_links[].multicast_pim_sm`.

The breaking change in default behavior is that for a P2P link, when `include_in_underlay_protocol: true`, then by default PIM SM is configured if enabled globally (whether by the node_settings or the global key).
...
+ underlay_mutlicast_pim_sm: true
 ...
 l3leafs:
   ...
   nodes:
     - name: MULTICAST_LEAF
+      # Alternatively, PIM SM can be enabled at node_setting level
+      underlay_multicast:
+        pim_sm:
+          enabled: true
