...
### `vlan_interfaces[].vrrp_ids[].ipv6.addresses` is required

In AVD 5.0.0, setting `addresses` key under `vlan_interfaces[].vrrp_ids[].ipv6` was not
required.

As of AVD 6.0.0, the `addresses` key is required and must be set to generate the configuration and documentation for `vlan_interfaces[].vrrp_ids[].ipv6`:
