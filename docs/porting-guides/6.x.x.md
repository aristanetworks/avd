...
### `management_eapi` will take effect only when it is explicitly enabled

In previous versions, `management_eapi.enabled` was `true` by default.
In AVD 6.0, this key should be set to true to set `management api http-commands` configuration.
...
### `mgmt_ip` or `ipv6_mgmt_ip` is required when using the default value of `management_eapi.vrfs`

Before AVD 6.x, it was not mandatory to set `mgmt_ip` or `ipv6_mgmt_ip` when using the default value of `management_eapi.vrfs`.
This has changed in AVD 6.0, `mgmt_ip` or `ipv6_mgmt_ip` must be set when using the default value of `mangement_eapi.vrfs`.
