...
### Change of default behavior for OSPF graceful restart for `underlay_ospf_graceful_restart`

In AVD 5.X.X, OSPF graceful restart set by `underlay_ospf_graceful_restart` was false by default.

As of AVD 6.0.0, `underlay_ospf_graceful_restart` default value change to true.
