...
### `errdisable.recovery.causes` is now a dictionary

In previous versions of AVD, `errdisable.recovery.causes` was a list of strings.
Starting AVD 6.0, this model is improved by supporting configuration of recovery interval for each cause. `errdisable.recovery.causes` is a now a list of dictionaries. Refer below diff to update the inputs according to new data model.

```diff
errdisable:
  recovery:
-   causes: [bgpguard, tapagg]
+   causes:
+     - name: bgpguard
+     - name: tapagg
```
