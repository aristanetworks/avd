...
Starting with AVD 6.0.0, warnings are emitted when top-level keys from the `eos_cli_config_gen` schema are found in the host_vars handed over to `eos_designs` role.
