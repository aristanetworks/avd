...
### `static_routes.prefix` and `ipv6_static_routes.prefix` are now required keys

In previous versions if 'prefix' was not defined, AVD generated incorrect configuration. To avoid this in AVD 6.0.0, `static_routes.prefix` and `ipv6_static_routes.prefix` are marked as required keys to generate static routes configuration.
