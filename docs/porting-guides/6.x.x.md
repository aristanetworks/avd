...
### eos_designs structured_configuration is now shared across eos_designs modules

In AVD 5.x, the generation of structured configuration was done python module per python module.
This implied that it was possible to declare conflicting information between modules (e.g. overwrite MLAG interface in connected_endpoint).
AVD was able to detect conflicting declaration within a module but not across modules which could lead to difficult to track errors.

Starting with AVD 6.0.0, the same structured configuration object is used across all python modules allowing for a much stronger detection of these conflicting requirements.
You may experience errors when migrating, but in general, these errors indicate that you had conflicting inputs across different configuration sections in your variables and you should adjust them accordingly.
