...
| `<network_services_keys.name>[].vrfs[].l3_port_channels[].member_interfaces[].name` | The pattern <code>Ethernet[\\d/]+</code> must now match the whole string |
| `<network_services_keys.name>[].vrfs[].l3_port_channels[].member_interfaces[].speed` | Added valid_values |
| `<network_services_keys.name>[].vrfs[].l3_port_channels[].name` | The pattern <code>Port-Channel[\\d/]+(\\.[\\d]+)?</code> must now match the whole string |
...
| `<node_type_keys.key>.defaults.isis_system_id_prefix` | The pattern <code>[0-9a-f]{4}\\.[0-9a-f]{4}</code> must now match the whole string |
| `<node_type_keys.key>.defaults.l3_interfaces[].name` | The pattern <code>Ethernet[\\d/]+(\\.[\\d]+)?</code> must now match the whole string |
| `<node_type_keys.key>.defaults.l3_interfaces[].speed` | Added valid_values |
| `<node_type_keys.key>.defaults.l3_port_channels[].name` | The pattern <code>Port-Channel[\\d/]+(\\.[\\d]+)?</code> must now match the whole string |
| `<node_type_keys.key>.defaults.l3_port_channels[].member_interfaces[].name` | The pattern <code>Ethernet[\\d/]+</code> must now match the whole string |
...
| `<node_type_keys.key>.defaults.uplink_interfaces[]` | The pattern <code>Ethernet.*</code> must now match the whole string |
| `<node_type_keys.key>.defaults.uplink_switch_interfaces[]` | The pattern <code>Ethernet[\\d/]+</code> must now match the whole string |
| `<node_type_keys.key>.defaults.uplink_switch_interface_speed` | Added valid_values |
| `<node_type_keys.key>.defaults.vtep_loopback` | The pattern <code>Loopback[\\d/]+</code> must now match the whole string |
| `<node_type_keys.key>.defaults.wan_ha.ha_interfaces` | The pattern <code>`Ethernet[\\d/]+</code> must now match the whole string |
...
| `monitor_telemetry_influx.destinations[].url` | The pattern <code>(http(s)?|udp|unix)://.+</code> must now match the whole string |
| `monitor_telemetry_influx.source_sockets[].url` | The pattern <code>(http(s)?|udp|unix)://.+</code> must now match the whole string |
