...
### Warnings for eos_cli_config_gen keys in eos_designs input data

Starting with AVD 6.0.0, warnings are emitted when top-level keys from the `eos_cli_config_gen` schema are found in the input variables handed over to `eos_designs` role.
These keys are ignored during `eos_cli_config_gen` processing when the role is used after `eos_designs` role.

This new validation helps identify keys that will not have any effect, contrary to previous AVD versions. By default, these warnings are enabled to help users identify potential configuration issues during transition.

For detailed information about this behavior, recommended solutions, and examples, see the [documentation on using eos_cli_config_gen keys with eos_designs](../warn-eos-cli-config-keys-usage-in-eos-designs.md).

If you want to disable these warnings, you can set the `avd_eos_designs_warn_eos_cli_config_gen_keys` variable to `false`:
...
!!! note
    Even when the warnings are disabled, the top-level `eos_cli_config_gen` keys in the input data will still be ignored during `eos_cli_config_gen` processing when run in conjunction with `eos_designs`.
