...
| `avd_6_behaviors` | - |
| `core_interfaces.p2p_links[].structured_config` | `core_interfaces.p2p_links.ethernet_structured_config` or `l3_edge.p2p_links.port_channel_structured_config` |
...
| `core_interfaces.p2p_links_profiles[].structured_config` | `core_interfaces.p2p_links_profiles.ethernet_structured_config` or `l3_edge.p2p_links_profiles.port_channel_structured_config` |
| `cvp_ingestauth_key` | - |
...
In AVD 5.0.0, p2p_links list keys nodes, ip, interfaces and descriptions did not have any limitation. This was sometimes working but could lead to undefined behavior as all the code was written to work for exactly two nodes, in particular when figuring out the peer device.

Starting AVD 6.0.0, the length to all these lists has been restricted to exactly 2 items (min_length: 2 and max_length: 2).
