...
### Removal of default Jinja2 templates in eos_designs

Since AVD 5.0, Jinja2 templates have not been the default method for configuring descriptions.

Starting AVD 6.0.0, the templates are completely removed from the eos_designs role. Custom templates may still be provided, but the role no longer includes built-in templates.

For migration cases where the legacy templates are required, they can be copied from AVD 5.7 into a custom location and referenced accordingly.
