...
| -------------- | -------------- |
| `ip_ftp_client_source_interfaces` | `ip_ftp_client` |
| `radius_server.hosts` | `radius_server.vrfs` |
| `ip_telnet_client_source_interfaces` | `ip_telnet_client` |
| `ip_tftp_client_source_interfaces` | `ip_tftp_client` |
...
#### `radius_server.hosts` data-model changed to `radius_server.vrfs` data-model

In AVD 5.x, the data model `radius_server.hosts` could generate single command using the same VRF for a particular host and when TLS was configured, the `key` was ignored.

Starting with AVD 6.0.0, `radius_server.hosts` has been replaced by `radius_server.vrfs`, where the VRF is now the primary key, under which multiple hosts can be configured and the TLS and `key` can be configured together.
