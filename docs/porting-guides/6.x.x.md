...
### Removal of redundant IPv4 neighbor deactivation under BGP

In AVD 5.x.x, the IPv4 address-family configuration could include explicit neighbor deactivation using the `no neighbor <peer-group> activate` command, even when IPv4 unicast was already disabled using `no bgp default ipv4-unicast` at the BGP process level.

Starting with AVD 6.0, this redundant `no neighbor <peer-group> activate` command under the IPv4 address-family is no longer rendered when IPv4 unicast is globally disabled using the `no bgp default ipv4-unicast` command. Since `no bgp default ipv4-unicast` already disables IPv4 unicast globally, explicitly deactivating neighbors under the IPv4 address-family has no functional effect.
