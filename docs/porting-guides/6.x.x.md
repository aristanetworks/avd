...
| `snmp_settings.users` | `snmp_settings.local_users` |
...
### Changes to the `snmp_settings.local_users` data-model

In AVD 5.x, SNMP users were configured under `snmp_settings.users` with a flat structure where `group` and `version` were required keys.

Starting with AVD 6.0.0, the `snmp_settings.users` key has been renamed to `snmp_settings.local_users`, and the structure has been updated to support multiple SNMP versions per user through version-specific group fields (`v1_group`, `v2c_group`, `v3`). SNMPv3 configuration is now nested under a `v3` dictionary.

To update the configuration, the following changes are required:

```diff
snmp_settings:
- users:
+ local_users:
    - name: USER-V3-AUTH-PRIV
-     group: GRP-READ-ONLY
-     version: v3
-     auth: sha
...
| `snmp_server.users` | `snmp_server.local_users or snmp_server.remote_users` |
...
### Changes to the `snmp_server` data-model for local and remote users

In AVD 5.x, all SNMP Local and Remote users shared the same structure and were defined under `snmp_server.users` key with a flat structure.

As of AVD 6.0.0, the data model has been updated with the following changes:

- The `snmp_server.users` key has been replaced with two explicit keys: `snmp_server.local_users` and `snmp_server.remote_users`
- The structure now supports multiple SNMP versions per user through version-specific group fields (`v1_group`, `v2c_group`, `v3`)
- SNMPv3 configuration is now nested under a `v3` dictionary

This change clearly separates local and remote SNMP users and better aligns the data-model with EOS behavior.
...
+       priv: aes
+       priv_passphrase: clearPassword
+   # A single user can be configured for multiple SNMP versions
+   - name: USER-MULTI-VERSION
+     v1_group: GRP-V1
+     v2c_group: GRP-V2C
+     v3:
+       group: GRP-V3
+       auth: sha
+       auth_passphrase: multiVersionPassword
+ remote_users:
+   - name: REMOTE-USER-IP-ONLY
+     v3:
+       group: GRP-REMOTE
...
| `snmp_server.engine_ids.remotes[].address` | required |
| `snmp_server.engine_ids.remotes[].id` | required |
