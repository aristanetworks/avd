...
### Removal of default `bgp_ecmp` value for non-WAN routers

In AVD 5.x, non-WAN routers had a default `bgp_ecmp` value of 4.

Starting AVD 6.0.0, the default `bgp_ecmp` value for non-WAN routers has been removed to take advantage of each platform defaults instead of potentially not installing paths. `bgp_ecmp` must be explicitly defined when required.
