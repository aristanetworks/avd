...
Starting AVD 6.0.0, `no-drop` has been removed from the valid-values for `priority_flow_control.watchdog.action`, as it is not supported by the EOS CLI. The valid values are now `drop`, `errdisable`, and `notify-only`.
