...
!!! note
    Even when warnings are disabled, the top-level `eos_cli_config_gen` keys in the input data will still be ignored during `eos_cli_config_gen` processing when run in conjunction with `eos_designs`.
