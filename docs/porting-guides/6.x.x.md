...
#### `underlay_ospf_authentication.message_digest_keys[].cleartext_key` is required

In AVD 5.x.x, setting `cleartext_key` key under `underlay_ospf_authentication.message_digest_keys[]` was not required.

As of AVD 6.0.0, the `cleartext_key` key is required and must be set to generate the configuration and documentation for `underlay_ospf_authentication.message_digest_keys[]`.
