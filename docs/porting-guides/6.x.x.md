...
### `ip_dhcp_snooping.enabled` is not a required key

In AVD 5.x, the value of `ip_dhcp_snooping.enabled` was implicitely required as true to configure 'ip dhcp snooping information option circuit-id type xx format xx' command.

In AVD 6.0.0, this command can be configured without setting `ip_dhcp_snooping.enabled` as show below
