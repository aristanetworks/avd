...
### The plugin argument `avd_catalogs_allow_bgp_vrfs` has been removed

The argument for BGP test inclusion has been removed. Instead of using the dedicated plugin argument `avd_catalogs_allow_bgp_vrfs`, use the new, `router_bgp.vrfs.neighbors.metadata.validate_state` under the structured_configurations key to enable BGP tests for non-default VRFs.
