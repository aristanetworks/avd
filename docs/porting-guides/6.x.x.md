...
If `bgp_as_notation` is set to `auto` or left undefined, the system examines its own AS number.
...
To retain the old behavior, set `bgp_as_notation` to `asplain`.

```diff
+ bgp_as_notation: asplain
