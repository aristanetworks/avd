...
In AVD 6.0.0, `pyavd.get_avd_facts` now accepts either a dictionary of `AVDDesign` dataclasses or a dictionary of dictionaries.
...
In AVD 6.0.0, `pyavd.get_device_structured_config` now returns an `EOSConfig` dataclass instead of a dictionary.
This dataclass holds the complete structured configuration for a device and can be used directly as input
