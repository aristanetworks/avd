...
Till AVD 5.x, All BGP AS would get converted to `asplain` in EOS even when it is given in `asdot` format in eos_designs input, since the option to set AS notation was not available in eos_designs.

Starting AVD 6.0.0, To configure BGP AS notation set `bgp_as_notation: auto/asdot/asplain` default is `auto`.
If `bgp_as_notation` is not defined or set to `auto`, it will check its own AS number if dot(.) is found in the input it will convert all the other AS numbers to `asdot` notation and if dot(.) is not found in the input it will convert all the other AS numbers to `asplain` notation.
