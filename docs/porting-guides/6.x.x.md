...
### Changes to the `snmp_server.users` data-model to separate local and remote users

In AVD 5.x, All SNMP Local and Remote users shared the same structure and were defined under `snmp_server.users` key.

As of AVD 6.0.0, The `snmp_server.users` key has been replaced with two explicit keys `snmp_server.local_users` and `snmp_server.remote_users` where `name`, `group` and `version` are the required keys.

This change clearly separates the local SNMP users and remote SNMP users and better aligns the data-model with EOS behavior.
