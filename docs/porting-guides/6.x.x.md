...
### Enable VRF to configure `IPV4` and `IPV6` ACLs

In AVD 5.x, we could render IPv4 / IPv6 ACLs even if the VRF enable was not set.

In AVD 6.x, it is required to enable VRF in order to configure IPV4/IPV6 ACLs or use custom structured config.
