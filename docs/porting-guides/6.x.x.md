...
Starting AVD 6.0.0, the default `bgp_ecmp` value for non-WAN routers is removed, and `bgp_ecmp` must be explicitly defined when required.

To retain the previous behavior, set the value explicitly in structured config:
