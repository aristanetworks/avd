...
In AVD 6.0.0, `pyavd.get_device_test_catalog` has undergone significant changes to improve type safety and shift logical responsibilities to the caller.
...
+ if dc1_leaf1a_structured_config.get("metadata", {}).get("is_deployed", True) is False:
+   anta_catalog = AntaCatalog()
+ else:
+   anta_catalog = get_device_test_catalog(hostname="DC1-LEAF1A", structured_config=dc1_leaf1a_structured_config, fabric_data=fabric_data)
```
...
**Action Required:** Instead of passing `InputFactorySettings(allow_bgp_vrfs=True)`, you must now set `metadata.validate_state` to `True` on specific BGP neighbors or neighbor interfaces (RFC 5549) within the structured configuration.

```diff
from pyavd import get_device_test_catalog
- from pyavd.api._anta import AvdFabricData, AvdCatalogGenerationSettings, InputFactorySettings
+ from pyavd.api._anta import AvdFabricData, AvdCatalogGenerationSettings
...
- anta_catalog = get_device_test_catalog(hostname="DC1-LEAF1A", structured_config=dc1_leaf1a_structured_config, fabric_data=fabric_data, settings=settings)
+ anta_catalog = get_device_test_catalog(hostname="DC1-LEAF1A", structured_config=dc1_leaf1a_structured_config, fabric_data=fabric_data)
```

#### 4. `AvdCatalogGenerationSettings` is now a Dataclass

`AvdCatalogGenerationSettings` has been converted from a Pydantic model to a standard Python `dataclass`.

**Action Required:** If you code relies on Pydantic-specific methods (like validation error handling or serialization), updates are required.
