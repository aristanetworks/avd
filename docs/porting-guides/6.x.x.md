...
### Removing of `uplink_structured_config` and introduction of uplink-specific structured config keys

In AVD 5.x, the key `uplink_structured_config` was used to apply custom structured configuration to uplink interfaces.  
Depending on the uplink type, the content could apply to either:

- `ethernet_interfaces` when `uplink_type == "p2p"`
- `port_channel_interfaces` when `uplink_type == "port-channel"`
- `uplink_switch_interfaces` on uplink switches

This generic approach required relaxed schema validation and made it unclear which interface type the configuration applied to.

Starting with AVD 6.0.0, `uplink_structured_config` has been removed and replaced by explicit, interface-specific keys to improve clarity and schema validation.
...
```diff
# Example: Point-to-point uplinks (`uplink_type: p2p`)
- <node_type_key>.defaults.uplink_structured_config:
-   description: Uplink to spine
-   mtu: 9214
+ <node_type_key>.defaults.uplink_ethernet_structured_config:
+   description: Uplink to spine
+   mtu: 9214
...
+ <node_type_key>.nodes[].uplink_port_channel_structured_config:
+   description: Uplink Port-Channel
+   lacp_fallback_timeout: 30

# Example: Port-Channel uplinks on uplink switches
- <node_type_key>.node_groups[].uplink_structured_config:
-   description: Downlink to leaf
-   mtu: 9214
+ <node_type_key>.node_groups[].uplink_switch_port_channel_structured_config:
+   description: Downlink to leaf
+   mtu: 9214
```

### Removing of `structured_config` and introduction of interface-specific structured config keys
...
### Change of SNMP `snmp_settings.enable_inband_mgmt_vrf` and `snmp_settings.enable_mgmt_interface_vrf` to `snmp_settingsvrfs[]`
