...
### Removal of `wan_use_agent_env_var_for_kernel_software_forwarding_ecmp` and introduction of `kernel_ecmp_cli` under `platform_settings`

In AVD 5.x, for EOS kernel forwarding, ECMP programming could be enabled using `wan_use_agent_env_var_for_kernel_software_forwarding_ecmp` key.

Starting AVD 6.0.0, the key `wan_use_agent_env_var_for_kernel_software_forwarding_ecmp` has been removed.
A new key `kernel_ecmp_cli` has been introduced under `platform_settings` to control the same behavior, with a default value of `true`.
