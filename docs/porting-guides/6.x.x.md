...
### All the traffic policies counters should be defined under `traffic_policies.policies[].counters`, it will no longer be added automatically

In AVD 5.0.0, Counters were generated automatically based on `traffic_policies.policies[].matches[].actions[].count`.

Starting AVD 6.0.0, all the counters should be defined under the new key `traffic_policies.policies[].counters`.
