...
In previous versions of AVD, the value of `maximum_routes` for underlay `bgp_peer_groups` was set to `12000` by default.

Starting AVD 6.0, the default value of `maximum_routes` for underlay `bgp_peer_groups` is set to `256000`.

To retain the previous behavior, `maximum_routes` can be set to `12000` under `bgp_peer_groups.ipv4_underlay_peers.structured_config.maximum_routes` for the particular peer-group name.
...
+    structured_config:
+      # overwrite maximum_routes default value for MLAG-IPv4-UNDERLAY-PEER
