...
These keys are ignored during `eos_cli_config_gen` processing when the role is used after `eos_designs` role (except when running with `read_structured_config_from_file: false` is set when running `eos_cli_config_gen`).
