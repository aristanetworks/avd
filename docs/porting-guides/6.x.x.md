...
AVD will now consistently raise errors when the same Structured Config object is defined with conflicting data in multiple input models.
...
Starting with AVD 6.0.0, `uplink_structured_config` has been removed and replaced by explicit, interface type-specific keys to improve clarity and schema validation. With this change it is now possible to apply structured config to uplinks and its peer interfaces with individually.
...
### Removal of `structured_config` and introduction of interface-specific structured config keys
