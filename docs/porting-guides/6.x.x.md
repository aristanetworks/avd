...
Starting with AVD 6.0.0, `radius_server.hosts` has been replaced by `radius_server.vrfs`, where the VRF is now the primary key, under which multiple hosts can be configured and `radius_server.servers` for the default VRF. It is now also possible to configure side-by-side TLS and non-TLS servers with the same host IP.
