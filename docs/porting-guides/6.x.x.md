...
Till AVD 5.x, It was allowed to configure the same value for `vrfs[].ospf.process_id` and `underlay_ospf_process_id` for any VRF.

Starting AVD 6.0.0, setting the same value for `vrfs[].ospf.process_id` and `underlay_ospf_process_id` will not be allowed for any VRF other than `default` vrf.

An error will be raised for each vrf having the same `ospf.process_id` as `underlay_ospf_process_id`.
