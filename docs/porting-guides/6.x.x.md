...
### Removal of setting `admin_subfield` to `router_id` on invalid IPv4 address entry

In AVD 5.x, setting invalid ipv4 address in `admin_subfield` would set the value to `router_id` ignoring a potential input typo.

Starting AVD 6.0.0, setting invalid ipv4 address in `admin_subfield` would raise a validation error.
