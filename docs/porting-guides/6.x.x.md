...
!!! note
    If Ansible Vault is configured, some files in the temporary directory may be encrypted.
    To inspect encrypted files, decryption is required. For example:

    ```bash
    ansible-vault view intended/tmp/eos_designs/validated/DC1-LEAF1A.json
    ```

    Please refer to the [Ansible documentation](https://docs.ansible.com/projects/ansible/latest/vault_guide/index.html) for more details.
