...
A new key `kernel_ecmp_cli` under `platform_settings` has been introduced to control this behavior with `default: true` i.e. the functionality is now enabled by default via the proper CLI.
...
In AVD 6.0.0, 802.1X configuration has been consolidated into a new `dot1x_settings` data model. Previously, configuring 802.1X required a combination of `custom_structured_configuration` (to enable the feature globally and configure it) and `aaa_settings` (to configure 802.1X Authentication, Authorization, and Accounting methods.)

The new model provides a centralized configuration and strict validation. Any RADIUS server group referenced in `dot1x_settings` must be explicitly defined under `aaa_settings.radius.servers`, otherwise an error will be raised.

The new model also introduces new default behaviors, such as automatically enabling the redistribution of static 802.1X-learned MAC addresses into EVPN (controlled via `dot1x_settings.redistribute_in_evpn`). If you previously configured `redistribute dot1x` manually under BGP using custom structured configuration, this should be removed.

To migrate, you must remove the relevant `dot1x` keys from `aaa_settings` as they have been **removed** from the schema.

Additionally, you should remove any `dot1x` custom structured configuration that is now handled by `dot1x_settings` to avoid conflicts.

!!! tip
    Since `dot1x_settings` is an abstraction, it may not cover every possible 802.1X configuration available in AVD. It is recommended to remove your existing `dot1x` custom structured configuration, generate the configuration using `dot1x_settings`, and review the output. You can then re-apply your custom structured configuration for any advanced settings that are not covered by the new model.
...
+   enabled: true
+   authentication:
+     radius_groups: [agni-server-group]
+   accounting:
+     radius_groups: [agni-server-group]
+     multicast: true
+     syslog: true
