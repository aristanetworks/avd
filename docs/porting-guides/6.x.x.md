...
### Valid values for `errdisable.detect.causes` and `errdisable.recovery.causes.name` updated

In AVD 6.0.0, the list of supported errdisable causes has been aligned with EOS 4.35.0F.
Several values present in AVD 5.x have been removed, and new granular 802.1X-related causes have been added.

Changes to `errdisable.detect.causes`:
...
Changes to `errdisable.recovery.causes.name`:
...
### The variable `anta_report_hide_statuses` has been renamed to `anta_report_exclude_statuses` for clarity
