...
### Changes in validation requirements for `cv_settings.onprem_clusters`

In AVD 5.x.x, `cv_settings.onprem_clusters` could be configured without enforcing the presence of NTP or DNS settings, even when CVP servers were defined using DNS names.

Starting AVD 6.0, additional validation is enforced when `cv_settings.onprem_clusters` is configured:

- `ntp_settings.servers` must be configured when `cv_settings.onprem_clusters` is defined.
- `dns_settings.servers` must be configured if any server under `cv_settings.onprem_clusters[].servers[].name` is specified using a DNS name.

These validations ensure that required infrastructure dependencies are explicitly configured when deploying CloudVision on-prem clusters.
