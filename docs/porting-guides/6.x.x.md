...
### `roles[].sequence_numbers[].sequence`, `roles[].sequence_numbers[].action` and `roles[].sequence_numbers[].command` now required to generate the configuration and documentation for `roles`
