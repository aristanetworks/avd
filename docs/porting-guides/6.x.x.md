...
In AVD 5.x, the conflict detection of AVD was done per data model (network_services, connected_enpoints, ...).
This implies that it was possible to declare conflicting information between data models (e.g. overwrite MLAG interface in connected_endpoints) which could lead to difficult to track errors.

Starting with AVD 6.0.0, conflict detection is now done across all data models allowing for a much stronger detection of these conflicting requirements.
