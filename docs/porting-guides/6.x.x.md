...
Starting AVD 6.0, new knobs `bgp_peer_groups.ipv4_underlay_peers.maximum_routes` and `bgp_peer_groups.mlag_ipv4_underlay_peer.maximum_routes` are introduced for setting maximum_routes, with a default value of `256000`.

To retain the previous behavior, `maximum_routes` can be set to `12000` under `bgp_peer_groups.ipv4_underlay_peers.maximum_routes` and `bgp_peer_groups.mlag_ipv4_underlay_peer.maximum_routes` for the particular peer-group.
