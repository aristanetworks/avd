...
Starting with AVD 6.0.0, `uplink_structured_config` has been removed and replaced by explicit, interface type-specific keys to improve clarity and schema validation. With this change it is now possible to apply structured config to uplinks and it's peer's interfaces individually.
