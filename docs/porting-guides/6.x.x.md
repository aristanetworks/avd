...
### `mgmt_ip` is required when using the default value of `management_eapi.vrfs`

Before AVD 6.x, it was not mandatory to set `mgmt_ip` when using default value of `management_eapi.vrfs`.
This has changed in AVD 6.0. `mgmt_ip` must be set to use default value of `mangement_eapi.vrfs`.
...
management_eapi:
+ enabled: true #This must be set to management_eapi to take effect.
  enable_https: true
