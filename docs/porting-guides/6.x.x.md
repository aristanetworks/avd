...
These keys are ignored during `eos_cli_config_gen` processing when the role is used after `eos_designs` role (except when running with `read_structured_config_from_file: false` is set when runnign `eos_cli_config_gen`).
...
By default, warnings are enabled to help identify configuration that will be ignored. If you want to disable these warnings, you can set the `avd_validation_configuration.warn_eos_config_keys` variable to `false`:
