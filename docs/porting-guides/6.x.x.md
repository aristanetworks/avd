...
| `vlan_interfaces[].ipv6_ospf_area` | `vlan_interfaces[].ipv6_ospf_process_identifier.ipv6_ospf_area` |
...
### `ipv6_ospf_area` moved under `ipv6_ospf_process_identifier` to support Process ID

In AVD 5.x, the OSPFv3 area was defined using the simple key `ipv6_ospf_area`. This structure prevented the definition of the OSPFv3 Process ID, often resulting in the generation of incomplete or incorrect EOS commands.

Starting with AVD 6.0.0, `ipv6_ospf_area` has been replaced by the dictionary `pv6_ospf_process_identifier`. This change allows defining both the id (Process ID) and `ipv6_ospf_area` explicitly, ensuring the correct command ipv6 ospf <process-id> area <area-id> is generated.
