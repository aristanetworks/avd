...
| `avd_6_behaviors` | - |
| `avd_eos_designs_debug` | `avd_debug` |
...
The `eos_designs` role now dumps facts to a JSON file in the AVD temporary directory (defined by `avd_tmp_dir`) instead of storing them in Ansible facts. This temporary directory is automatically cleaned up at the end of the role.

To retain access to these facts, set `avd_debug: true`. This prevents the cleanup of the temporary directory, allowing you to inspect facts data.
...
- name: Load AVD facts from temporary directory
  ansible.builtin.include_vars:
    file: "{{ avd_tmp_dir }}/eos_designs/eos_designs_facts.json"
    name: avd_switch_facts
```

!!! note
    If Ansible Vault is configured, the files in the temporary directory are encrypted. The `include_vars` module will automatically decrypt the files if the Vault secret is available.

### Renaming of `avd_eos_designs_debug` to `avd_debug`
...
Starting with AVD 6.0.0, this variable has been renamed to `avd_debug`.
...
The `eos_designs` role now dumps templated inputs, validated inputs, and facts data to JSON files in the AVD temporary directory (defined by `avd_tmp_dir`). This temporary directory is automatically cleaned up at the end of the role.

To retain access to these files, set `avd_debug: true`. This prevents the cleanup of the temporary directory, allowing you to inspect the data used by `eos_designs` for debugging purposes.

!!! note
    `avd_debug` now also works for the `eos_cli_config_gen` role.

!!! note
    If Ansible Vault is configured, the files in the temporary directory are encrypted.
    To inspect the files, decryption is required. Please refer to the [Ansible documentation](https://docs.ansible.com/projects/ansible/latest/vault_guide/index.html) for more information on how to decrypt files.
