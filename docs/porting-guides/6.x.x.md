...
### `underlay_ospf_maximum_paths`: Default number of OSPF next-hops in an ECMP route set to 128
...
Starting with AVD 6.0.0, this parameter now defaults to 128.

The difference shown below keeps the previous configurations.

```diff
+ underlay_ospf_maximum_paths: Null
```
