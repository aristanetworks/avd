...
### For EOS kernel forwarding, ECMP programming is not enabled by default

In AVD 5.x the default value of `wan_use_agent_env_var_for_kernel_software_forwarding_ecmp` was `true`.

Starting with AVD 6.0.0, the default value of `wan_use_agent_env_var_for_kernel_software_forwarding_ecmp` has been changed to `false`.
