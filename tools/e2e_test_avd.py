...
class InlineExecutor(Executor):
    """Execute submitted functions immediately in the current process."""

    def submit(self, fn: typing.Callable[..., typing.Any], /, *args: typing.Any, **kwargs: typing.Any) -> Future[typing.Any]:
...
class AvdV6Build:
    ...
    def common_build_stage(self) -> bool:
        ...
        try:
            avd_facts = get_avd_facts_for_fabric(self.config, self._loaded_avd_designs, self._validated_inputs_dict, pool_manager)
...
def get_avd_facts_for_fabric(
    config: FabricConfig,
    all_inputs: dict[str, AVDDesign],
    all_hostvars: dict[str, dict[str, typing.Any]],
    pool_manager: PoolManager,
) -> dict[str, typing.Any]:
    """Generate AVD facts using the resolved end-to-end fabric configuration."""
...
def get_structured_config_for_device(
    device: str,
    device_avd_validated_inputs: AVDDesign,
    device_avd_validated_inputs_dict: dict[str, typing.Any],
    avd_facts: dict[str, typing.Any],
    config: FabricConfig,
) -> dict[str, typing.Any]:
    """Generate structured configuration using the resolved end-to-end fabric configuration."""
...
def build_validate_and_render_for_one_device(
    ...
    if config.avd_design:
        ...
        try:
            eos_config = get_structured_config_for_device(device, device_avd_validated_inputs, device_avd_validated_inputs_dict, avd_facts, config)
        except Exception as e:
