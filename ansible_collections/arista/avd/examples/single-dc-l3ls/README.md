...
This will show in the following:
...
- ansible-avd-examples/ (where you ran the playbook)
  - single-dc-l3ls
    - documentation/
    - group_vars/
    - images/
...
In the `ansible-avd-examples/single-dc-l3ls/group_vars/FABRIC.yml` file, we define generic settings that apply to all children of the `FABRIC` group as defined in the inventory described earlier.
...
In the `ansible-avd-examples/single-dc-l3ls/group_vars/DC1.yml` file, we define settings that apply to all children of the `DC1` group as defined in the inventory described earlier:
...
This time the settings defined are no longer fabric-wide, but are limited to DC1. In this example where we only have a single data center this is of limited benefit, but it allows us to scale our configuration to a scenario with multiple data centers in the future.

Initially we define the default gateway for the management interface of all devices in DC1.

The following section defines the spine layer:

- A number of default settings that apply to all nodes with the device type `spine` are defined:
  - `platform` allows us to reference default settings defined in AVD that are specific to certain switch platforms.
...
- `nodes` defines the actual spine switches, using the hostnames defined in the inventory.
  ...
  - `mgmt_ip` defines the IPv4 address of the management interface. As stated earlier Ansible will perform name lookup when using the hostnames specified in the inventory. However there is no automatic mechanism to grab the result of the name lookup and use that to generate the configuration of the actual management interface, hence we must specify it here.

Next we do the same, but this time for the L3 leaf switches. As you can see, significantly more settings need to be set compared to the spine switches:
...
- Again a number of default settings are defined:
  - `platform` allows us to reference default settings defined in AVD that are specific to certain switch platforms.
  - `loopback_ipv4_pool` defines the IP scope from which AVD assigns IPv4 addresses for Loopback0. Please note that in this example, this IP pool is identical to the one used for the spine switches. To avoid assigning identical IP addresses for several devices, we use the next option `loopback_ipv4_offset`.
  - `loopback_ipv4_offset` offsets the assigned loopback IP addresses from the beginning of the IP scope. This is required when the same IP pool is used for two different node_types (like spine and l3leaf in this example) to avoid over-lapping IPs. In this case the offset is "2" because we are using one loopback IP address for each of the spine switches.
  ...
  - `uplink_ipv4_pool` defines the IP scope from which AVD assigns IPv4 addresses for the uplink interfaces that we just defined.
  ...
  - `spanning_tree_priority` sets the spanning-tree priority. Since spanning-tree in a L3LS network is effectively only running locally on the switch, we can re-use the same priority across all L3 leaf switches.
...
Let's take a closer look at the `ansible-avd-examples/single-dc-l3ls/group_vars/NETWORK_SERVICES.yml` file shown below.
...
Each VLAN has a name and a virtual IP address assigned, which will be used as the default gateway for that particular VLAN.
...
At the very bottom of the `NETWORK_SERVICES.yml` file we define two layer2-only VLANs (`VLAN3401` and `VLAN3402`). These VLANs are only bridged across the fabric, but never routed anywhere:
...
As we saw in the previous section, we've now defined all VRFs and VLANs across the fabric. Let's take a closer look at the `ansible-avd-examples/single-dc-l3ls/group_vars/CONNECTED_ENDPOINTS.yml` file, which specifies the connectivity for all endpoints in the fabric (typically servers):
...
If we look closer at the definitions for `dc1-leaf1-server1`, we see the following:
...
- The relevant `adapters` are defined:
  ...
  - `server_ports` are defined purely for use in the interface descriptions on the switch. This doesn't configure anything on the server.
  ...
  - `vlans` define which VLANs are allowed on the switch_ports, in this case it's two ranges, VLAN11-12 and VLAN21-22 for the dual-attached server ports and VLAN11 for the iLO port.
...
- `generate intended variables` uses the role `arista.avd.eos_designs`, which will generate structured configuration for each device. This structured configuration can be found in the `ansible-avd-examples/single-dc-l3ls/intended/structured_configs` folder.
...
By simply commenting out (or deleting) the third and last task definition, then the playbook will generate all of the output (variables, configurations, documentation), but will not attempt to communicate with any devices.
