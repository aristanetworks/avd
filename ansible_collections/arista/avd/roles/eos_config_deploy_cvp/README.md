...
- **`apply_mode`**: `loose` / `strict`. Ony applicable with `cv_collection=v3`. If set to `strict`, all configlets not listed in your vars will be detached. The default is `loose`.

!!! warning
    cv_collection v1 will be deprecated in AVD 4.0, it's highly recommended users move to the v3 collection.
