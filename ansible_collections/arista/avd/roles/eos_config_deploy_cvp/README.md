...
- __`container_root`__: Inventory group name where Fabric devices are located. Default: `{{ fabric_name }}`.
- __`configlets_prefix`__: Prefix to use for configlet on CV side. Default: `AVD-{{ fabric_name }}-`.
- __`device_filter`__: Filter to target a specific set of devices on CV side. Default: `all`. It can be either a string or a list of strings.
...
- __`cv_collection`__: Version of Cloudvision collection to use.
