...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;- types</samp>](## "default_interfaces.[].types") | List, items: String | Required |  |  | List of node type keys.<br>'default' will apply these default interfaces to node types where there is no more-specific match.<br>Note that the node's platform must still match a platform type regular expression.<br> |
