...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;uplink_port_channel_id</samp>](## "<node_type_keys.key>.defaults.uplink_port_channel_id") | Integer |  |  | Min: 1<br>Max: 999999 | Only applicable when uplink_type == "port-channel".<br>By default the uplink Port-channel ID will be set to the number of the lowest member interface defined in the uplink_interfaces.<br>For example:<br>  member ports [ Eth22, Eth23 ] -> ID 22<br>  member ports [ Eth11/1, Eth22/1 ] -> ID 111<br>For MLAG port-channels ID will be based on the lowest member interface on the first MLAG switch.<br>This option overrides the default behavior and statically sets the local Port-channel ID.<br>Note! Make sure that an ID is used which does not overlap with the autogenerated Port-channel IDs in the Network Services.<br>Note! For MLAG pairs IDs between 1 and 2000 are supported.<br> |
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;uplink_switch_port_channel_id</samp>](## "<node_type_keys.key>.defaults.uplink_switch_port_channel_id") | Integer |  |  | Min: 1<br>Max: 999999 | Only applicable when uplink_type == "port-channel".<br>By default the uplink switch Port-channel ID will be set to the number of the first interface defined under `uplink_switch_interfaces`.<br>For MLAG this is taken from the first MLAG switch.<br>For example:<br>  member ports [ Eth22, Eth23 ] -> ID 22<br>  member ports [ Eth11/1, Eth22/1 ] -> ID 111<br>For MLAG port-channels ID will be based on the lowest member interface on the first MLAG switch.<br>This option overrides the default behavior and statically sets the uplink switch Port-channel ID.<br>Note! Make sure that an ID is used which does not overlap with the autogenerated Port-channel IDs in the Network Services.<br>Note! For MLAG pairs IDs between 1 and 2000 are supported.<br>WARNING!<br>  Using this variable as the default for a L2-switch may break configuration on the uplink switch by "collapsing" port-channel IDs.<br>  Please use this feature with caution, assign unique uplink_switch_port_channel_id for every L2-switch and review configuration produced by AVD.<br> |
...
=== "YAML"
    ...
    <node_type_keys.key>:
      ...
      defaults:
        ...
        # Only applicable when uplink_type == "port-channel".
        # By default the uplink Port-channel ID will be set to the number of the lowest member interface defined in the uplink_interfaces.
        # For example:
        ...
        # Note! Make sure that an ID is used which does not overlap with the autogenerated Port-channel IDs in the Network Services.
        # Note! For MLAG pairs IDs between 1 and 2000 are supported.
        ...
        # For MLAG this is taken from the first MLAG switch.
        # For example:
        #   member ports [ Eth22, Eth23 ] -> ID 22
        #   member ports [ Eth11/1, Eth22/1 ] -> ID 111
        # For MLAG port-channels ID will be based on the lowest member interface on the first MLAG switch.
        # This option overrides the default behavior and statically sets the uplink switch Port-channel ID.
        # Note! Make sure that an ID is used which does not overlap with the autogenerated Port-channel IDs in the Network Services.
        # Note! For MLAG pairs IDs between 1 and 2000 are supported.
        # WARNING!
        #   Using this variable as the default for a L2-switch may break configuration on the uplink switch by "collapsing" port-channel IDs.
        #   Please use this feature with caution, assign unique uplink_switch_port_channel_id for every L2-switch and review configuration produced by AVD.
