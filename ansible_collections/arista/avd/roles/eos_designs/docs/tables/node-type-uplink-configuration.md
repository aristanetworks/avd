...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;uplink_port_channel_id</samp>](## "<node_type_keys.key>.defaults.uplink_port_channel_id") | Integer |  |  | Min: 1<br>Max: 999999 | Only applicable when uplink_type == "port-channel".<br>By default the uplink Port-channel ID will be set to the number of the lowest member interface defined in the uplink_interfaces.<br>This option overrides the default behavior and statically sets the local Port-channel ID.<br>Note! Make sure that an ID is used which does not overlap with the autogenerated Port-channel IDs in the Network Services.<br>Note! For MLAG pairs IDs between 1 and 2000 are supported.<br> |
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;uplink_switch_port_channel_id</samp>](## "<node_type_keys.key>.defaults.uplink_switch_port_channel_id") | Integer |  |  | Min: 1<br>Max: 999999 | Only applicable when uplink_type == "port-channel".<br>By default the uplink switch Port-channel ID will be set to the number of the lowest member interface defined in the uplink_switch_interfaces.<br>This option overrides the default behavior and statically sets the uplink switch Port-channel ID.<br>Note! Make sure that an ID is used which does not overlap with the autogenerated Port-channel IDs in the Network Services.<br>Note! For MLAG pairs IDs between 1 and 2000 are supported.<br> |
...
=== "YAML"
    ...
    <node_type_keys.key>:
      ...
      defaults:
        ...
        # Only applicable when uplink_type == "port-channel".
        # By default the uplink switch Port-channel ID will be set to the number of the lowest member interface defined in the uplink_switch_interfaces.
