...
=== "Table"
    ...
    | [<samp>shutdown_bgp_towards_undeployed_peers</samp>](## "shutdown_bgp_towards_undeployed_peers") | Boolean |  | `True` |  | Administratively shuts down BGP neighbors on deployed devices that peer with any device marked with `is_deployed: false`. |
    | [<samp>shutdown_interfaces_towards_undeployed_peers</samp>](## "shutdown_interfaces_towards_undeployed_peers") | Boolean |  | `True` |  | Administratively shuts down interfaces on deployed devices that connect to any peer marked with `is_deployed: false`. |
...
=== "YAML"
    ...
    # Administratively shuts down BGP neighbors on deployed devices that peer with any device marked with `is_deployed: false`.
    shutdown_bgp_towards_undeployed_peers: <bool; default=True>

    # Administratively shuts down interfaces on deployed devices that connect to any peer marked with `is_deployed: false`.
