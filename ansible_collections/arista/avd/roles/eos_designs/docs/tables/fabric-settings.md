...
=== "Table"
    ...
    | [<samp>shutdown_bgp_towards_undeployed_peers</samp>](## "shutdown_bgp_towards_undeployed_peers") | Boolean |  | `False` |  | When a device is set undeployed using `is_deployed: false` and `shutdown_bgp_towards_undeployed_peers` key is set to true, the bgp neighborship beetween undeployed device and its peer is shut on the peer. |
...
=== "YAML"
    ...
    # When a device is set undeployed using `is_deployed: false` and `shutdown_bgp_towards_undeployed_peers` key is set to true, the bgp neighborship beetween undeployed device and its peer is shut on the peer.
