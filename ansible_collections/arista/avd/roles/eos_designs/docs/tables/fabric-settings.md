...
=== "Table"
    ...
    | [<samp>shutdown_bgp_towards_undeployed_peers</samp>](## "shutdown_bgp_towards_undeployed_peers") | Boolean |  | `True` |  | Administratively shuts down BGP neighbors on deployed devices that peer with a device marked with `is_deployed: false`. |
...
=== "YAML"
    ...
    # Administratively shuts down BGP neighbors on deployed devices that peer with a device marked with `is_deployed: false`.
