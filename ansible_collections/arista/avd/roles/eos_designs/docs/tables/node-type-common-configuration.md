...
=== "YAML"
    ...
    <node_type_keys.key>:
      ...
      defaults:
        ...
        # When set to 'dhcp' and avd_design_future.accept_dhcp_default_route_for_mgmt_ip_dhcp set to true, the `mgmt_destination_networks` and `mgmt_gateway` settings will be ignored since the DHCP server is expected to provide the gateway and routes.
        mgmt_ip: <str>

        # This key sets the management gateway for the device. It takes precedence over the global `mgmt_gateway`.
        # This setting is ignored when 'mgmt_ip' is set to 'dhcp' and avd_design_future.accept_dhcp_default_route_for_mgmt_ip_dhcp is set to true, since the DHCP server will provide the gateway.
        mgmt_gateway: <str>
