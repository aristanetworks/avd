...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;static_routes</samp>](## "<network_services_keys.name>.[].vrfs.[].l3_interfaces.[].static_routes") | List, items: Dictionary |  |  |  | Static routes to be configured on every device when this interface is configured. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;simple_auth_key</samp>](## "<network_services_keys.name>.[].vrfs.[].l3_interfaces.[].ospf.simple_auth_key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password.<br><br>NOTE: when using type 7 password, the l3_interfaces.interfaces list must not be more than 1 interface<br>      or they must all be the same (e.g. [Ethernet7, Ethernet7]) as the type7 password depends on the interface<br>      name (when common encryption is disabled). |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;key</samp>](## "<network_services_keys.name>.[].vrfs.[].l3_interfaces.[].ospf.message_digest_keys.[].key") | String |  |  |  | Key for OSPF message digest.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 16 characters for cleartext password.<br><br>NOTE: when using type 7 password, the l3_interfaces.interfaces list must not be more than 1 interface<br>or they must all be the same (e.g. [Ethernet7, Ethernet7]) as the type7 password depends on the interface name<br>(when common encryption is disabled). |
            ...
            l3_interfaces:
              - interfaces:
                ...
                ospf:
                  ...
                  #
                  # NOTE: when using type 7 password, the l3_interfaces.interfaces list must not be more than 1 interface
                  #       or they must all be the same (e.g. [Ethernet7, Ethernet7]) as the type7 password depends on the interface
                  ...
                  message_digest_keys:
                    - id: <int>
                      ...
                      #
                      # NOTE: when using type 7 password, the l3_interfaces.interfaces list must not be more than 1 interface
                      # or they must all be the same (e.g. [Ethernet7, Ethernet7]) as the type7 password depends on the interface name
