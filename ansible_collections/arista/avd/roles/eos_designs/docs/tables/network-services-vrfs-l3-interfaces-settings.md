...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ipv6_addresses</samp>](## "<network_services_keys.name>.[].vrfs.[].l3_interfaces.[].ipv6_addresses") | List, items: String |  |  |  | IPv6 addresses for each node in `nodes`, e.g., '2001:db8::1/64'.<br>Length must match `nodes`. Can be used instead of or together with `ip_addresses`.<br>For subinterfaces, at least one of `ip_addresses` or `ipv6_addresses` must be set. |
