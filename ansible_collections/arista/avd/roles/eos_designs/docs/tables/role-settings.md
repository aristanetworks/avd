...
=== "Table"
    ...
    | [<samp>avd_tmp_dir</samp>](## "avd_tmp_dir") | String |  |  |  | Path for the AVD temporary directory.<br>Contains templated inputs, validated inputs, and facts data used internally by AVD plugins.<br>Defaults to 'intended/tmp'. The temporary directory is cleaned up at the end of each role.<br>If Ansible Vault is configured, the files in this directory are encrypted.<br> |
...
=== "YAML"
    ...
    # If Ansible Vault is configured, the files in the temporary directory are encrypted and decryption is required to inspect them.
