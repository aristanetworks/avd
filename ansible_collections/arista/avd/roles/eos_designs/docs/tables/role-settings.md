...
=== "Table"
    ...
    | [<samp>avd_tmp_dir</samp>](## "avd_tmp_dir") | String |  |  |  | Path for the AVD temporary directory.<br>Contains templated inputs, validated inputs, and facts data used internally by AVD plugins.<br>Defaults to 'intended/tmp'. The temporary directory is cleaned up at the end of each role.<br>When Ansible Vault is configured, temporary files holding input variables are encrypted.<br> |
...
=== "YAML"
    ...
    # When Ansible Vault is configured, temporary files holding input variables are encrypted.
    avd_tmp_dir: <str>
