...
=== "Table"
    ...
    | [<samp>avd_vault_id</samp>](## "avd_vault_id") | String |  |  |  | Vault identity to use for encrypting temporary files when Ansible Vault is configured.<br>When Ansible Vault is not configured, this parameter has no effect and files are written as plain JSON.<br>When Ansible Vault is configured AVD encrypts temporary files containing templated and validated data<br>to prevent sensitive information from being exposed in temporary directories.<br>  * when `avd_vault_id` is not specified, AVD uses the *first* vault identity in the list for encryption.<br>  * when `avd_vault_id` is specified, AVD uses the specified vault identity for encryption. |
...
=== "YAML"
    ...
    # When Ansible Vault is configured AVD encrypts temporary files containing templated and validated data
    # to prevent sensitive information from being exposed in temporary directories.
    #   * when `avd_vault_id` is not specified, AVD uses the *first* vault identity in the list for encryption.
    #   * when `avd_vault_id` is specified, AVD uses the specified vault identity for encryption.
    avd_vault_id: <str>
