...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;enabled</samp>](## "sflow_settings.export_to_cloudvision.enabled") | Boolean |  |  |  | When set to `true`, AVD automatically:<br>- Configures an sFlow destination at `127.0.0.1` port `6343` in the VRF determined by `export_to_cloudvision.vrf`.<br>- Configures the TerminAttr daemon (`daemon terminattr`) to export sFlow to the same `export_to_cloudvision.vrf/127.0.0.1:6343` in the selected VRF. |
...
=== "YAML"
    ...
    sflow_settings:
      ...
      export_to_cloudvision:
        ...
        # - Configures an sFlow destination at `127.0.0.1` port `6343` in the VRF determined by `export_to_cloudvision.vrf`.
        # - Configures the TerminAttr daemon (`daemon terminattr`) to export sFlow to the same `export_to_cloudvision.vrf/127.0.0.1:6343` in the selected VRF.
        enabled: <bool>
