...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;preserve_svi_vrf_default_prefix_list_order</samp>](## "avd_design_future.preserve_svi_vrf_default_prefix_list_order") | Boolean |  | `False` |  | Available from AVD 6.4.0.<br>Preserve insertion order for SVI subnets in the generated `PL-SVI-VRF-DEFAULT` prefix-list.<br>When disabled, AVD keeps the legacy behavior where prefixes are deduplicated with a set and then converted back to a list without sorting.<br>Since set ordering is not stable, this deduplication can lead to `PL-SVI-VRF-DEFAULT` changes across AVD runs. |
...
=== "YAML"
    ...
    avd_design_future:
      ...
      # Available from AVD 6.4.0.
      # Preserve insertion order for SVI subnets in the generated `PL-SVI-VRF-DEFAULT` prefix-list.
      # When disabled, AVD keeps the legacy behavior where prefixes are deduplicated with a set and then converted back to a list without sorting.
      # Since set ordering is not stable, this deduplication can lead to `PL-SVI-VRF-DEFAULT` changes across AVD runs.
      preserve_svi_vrf_default_prefix_list_order: <bool; default=False>
