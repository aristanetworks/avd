...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;allow_infinite_profile_inheritance</samp>](## "avd_design_future.allow_infinite_profile_inheritance") | Boolean |  | `False` |  | Available from AVD 6.5.0.<br>Allow `parent_profile` to inherit from its own `parent_profile` in `port_profiles`, `device_profiles` and `svi_profiles`. |
...
=== "YAML"
    ...
    avd_design_future:
      ...
      # Allow `parent_profile` to inherit from its own `parent_profile` in `port_profiles`, `device_profiles` and `svi_profiles`.
