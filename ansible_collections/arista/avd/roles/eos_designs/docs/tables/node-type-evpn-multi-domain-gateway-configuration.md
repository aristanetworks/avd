    ...
    <node_type_keys.key>:
      ...
      defaults:
        ...
        evpn_gateway:
          ...
          # If a remote peer is also an EVPN Route Server and uses the same IP address, only the EVPN Gateway core peering is configured on the local node.
          # The EVPN Route Server still configures the regular EVPN peering back to the local node.
          # If an explicit `ip_address` differs from the EVPN Route Server peering address, the EVPN Gateway core peering is configured in addition to the regular EVPN Route Server peering.
          remote_peers:
