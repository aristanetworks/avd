    ...
    <node_type_keys.key>:
      ...
      defaults:
        ...
        evpn_gateway:
          ...
          # When `avd_design_future.raise_for_evpn_gateway_remote_peers_without_evpn` is set to `true`, the local node must have EVPN overlay enabled when `remote_peers` is configured.
