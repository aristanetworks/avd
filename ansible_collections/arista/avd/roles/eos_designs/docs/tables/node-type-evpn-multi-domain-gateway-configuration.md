...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;remote_peers</samp>](## "<node_type_keys.key>.defaults.evpn_gateway.remote_peers") | List, items: Dictionary |  |  |  | Define remote peers of the EVPN VXLAN Gateway.<br>If the hostname can be found in the inventory, ip_address and BGP ASN will be automatically populated. Manual override takes precedence.<br>If the peer's hostname can not be found in the inventory, ip_address and bgp_as must be defined.<br>If a remote peer is also an EVPN Route Server or Route Server client and uses the same IP address, only the EVPN Gateway core peering is configured on the local node.<br>Suppression is evaluated independently on each node based on its local EVPN Route Server and client relationships and `evpn_gateway.remote_peers` configuration.<br>When two nodes have a regular EVPN Route Server/client relationship, but only one establishes a same-address core peering, the regular EVPN neighbor retained on the other node uses its locally configured `bgp_peer_groups.evpn_overlay_core` password.<br>If an explicit `ip_address` differs from the regular EVPN peering address, the EVPN Gateway core peering is configured in addition to the regular EVPN peering.<br> |
    ...
    <node_type_keys.key>:
      ...
      defaults:
        ...
        evpn_gateway:
          ...
          # If a remote peer is also an EVPN Route Server or Route Server client and uses the same IP address, only the EVPN Gateway core peering is configured on the local node.
          # Suppression is evaluated independently on each node based on its local EVPN Route Server and client relationships and `evpn_gateway.remote_peers` configuration.
          # When two nodes have a regular EVPN Route Server/client relationship, but only one establishes a same-address core peering, the regular EVPN neighbor retained on the other node uses its locally configured `bgp_peer_groups.evpn_overlay_core` password.
          # If an explicit `ip_address` differs from the regular EVPN peering address, the EVPN Gateway core peering is configured in addition to the regular EVPN peering.
          remote_peers:
