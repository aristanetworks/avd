...
=== "YAML"
    ...
    ssh_settings:
      vrfs:
          ...
          # - `use_mgmt_interface_vrf` will configure the SSH ACL under the VRF set with `mgmt_interface_vrf`.
          #   An error will be raised if `mgmt_ip` or `ipv6_mgmt_ip` are not configured for the device.
          # - `use_inband_mgmt_vrf` will configure the SSH ACL under the VRF set with `inband_mgmt_vrf`.
