...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;mlag</samp>](## "<node_type_keys.key>.defaults.mlag") | Boolean |  | `True` |  | Enable / Disable auto MLAG, when two nodes are defined in node group.<br>When using shared node groups with `avd_design_future.allow_mlag_in_shared_node_groups: true`,<br>  the order of this list determines MLAG role assignment for the two nodes with effective `mlag: true`:<br>  - The first node is the MLAG primary.<br>  - The second node is the MLAG secondary. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;mlag</samp>](## "<node_type_keys.key>.node_groups.[].nodes.[].mlag") | Boolean |  | `True` |  | Enable / Disable auto MLAG, when two nodes are defined in node group.<br>When using shared node groups with `avd_design_future.allow_mlag_in_shared_node_groups: true`,<br>  the order of this list determines MLAG role assignment for the two nodes with effective `mlag: true`:<br>  - The first node is the MLAG primary.<br>  - The second node is the MLAG secondary. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;mlag</samp>](## "<node_type_keys.key>.node_groups.[].mlag") | Boolean |  | `True` |  | Enable / Disable auto MLAG, when two nodes are defined in node group.<br>When using shared node groups with `avd_design_future.allow_mlag_in_shared_node_groups: true`,<br>  the order of this list determines MLAG role assignment for the two nodes with effective `mlag: true`:<br>  - The first node is the MLAG primary.<br>  - The second node is the MLAG secondary. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;mlag</samp>](## "<node_type_keys.key>.nodes.[].mlag") | Boolean |  | `True` |  | Enable / Disable auto MLAG, when two nodes are defined in node group.<br>When using shared node groups with `avd_design_future.allow_mlag_in_shared_node_groups: true`,<br>  the order of this list determines MLAG role assignment for the two nodes with effective `mlag: true`:<br>  - The first node is the MLAG primary.<br>  - The second node is the MLAG secondary. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;mlag</samp>](## "device_profiles.[].mlag") | Boolean |  | `True` |  | Enable / Disable auto MLAG, when two nodes are defined in node group.<br>When using shared node groups with `avd_design_future.allow_mlag_in_shared_node_groups: true`,<br>  the order of this list determines MLAG role assignment for the two nodes with effective `mlag: true`:<br>  - The first node is the MLAG primary.<br>  - The second node is the MLAG secondary. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;mlag</samp>](## "devices.[].mlag") | Boolean |  | `True` |  | Enable / Disable auto MLAG, when two nodes are defined in node group.<br>When using shared node groups with `avd_design_future.allow_mlag_in_shared_node_groups: true`,<br>  the order of this list determines MLAG role assignment for the two nodes with effective `mlag: true`:<br>  - The first node is the MLAG primary.<br>  - The second node is the MLAG secondary. |
...
=== "YAML"
    ...
    <node_type_keys.key>:
      ...
      defaults:
        ...
        #   the order of this list determines MLAG role assignment for the two nodes with effective `mlag: true`:
