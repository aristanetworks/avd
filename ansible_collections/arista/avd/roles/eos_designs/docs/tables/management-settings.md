...
=== "Table"
    ...
    | [<samp>inband_ztp_bootstrap_file</samp>](## "inband_ztp_bootstrap_file") | String |  |  |  | Bootstrap URL configured in DHCP to use for inband ZTP.<br>If not set and `cv_settings.onprem_clusters[0].servers[0].name` is set then the bootstrap value will be set to:<br>    `https://{cv_settings.onprem_clusters[0].servers[0].name}/ztp/bootstrap`<br>Otherwise no value will be configured. |
...
=== "YAML"
    ...
    # If not set and `cv_settings.onprem_clusters[0].servers[0].name` is set then the bootstrap value will be set to:
