...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;default_wan_role</samp>](## "node_type_keys.[].default_wan_role") | String |  |  | Valid Values:<br>- <code>client</code><br>- <code>server</code> | Set the default WAN role.<br><br>This is used both for AutoVPN and Pathfinder designs.<br>That means if `wan_mode` root key is set to `autovpn` or `sdwan`.<br>`server` indicates that the router is a route-reflector.<br><br>Only supported if `overlay_routing_protocol` is set to `ibgp`.<br> |
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;default_sdwan_role</samp>](## "node_type_keys.[].default_sdwan_role") | String |  |  | Valid Values:<br>- <code>edge</code><br>- <code>transit</code><br>- <code>pathfinder</code> | Set the default SDWAN role.<br><br>This key is used for Pathfinder designs only when the `wan_mode` root<br>key is set to `sdwan`.<br><br>`pathfinder` is only a valid id `wan_role` is `server`.<br>`edge` and `transit` are only valid if `wan_role` is `client`.<br> |
