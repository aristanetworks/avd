...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;uplink_type</samp>](## "node_type_keys.[].uplink_type") | String |  | `p2p` | Valid Values:<br>- <code>p2p</code><br>- <code>port-channel</code><br>- <code>p2p-vrfs</code> | `uplink_type` must be `p2p` if `vtep` or `underlay_router` is true.<br><br>For `p2p-vrfs`, the uplink are configured as an L3 interface with a subinterface for each VRF<br>in `network_services` present on both the uplink and the downlink switch.<br>The subinterface ID is the `vrf_id`.<br>The default VRF is configured on the physical interface.<br>All interfaces use the same IP address on each end. |
...
=== "YAML"
    ...
    node_type_keys:
      - key: <str; required; unique>
        ...
        # For `p2p-vrfs`, the uplink are configured as an L3 interface with a subinterface for each VRF
        # in `network_services` present on both the uplink and the downlink switch.
        # The subinterface ID is the `vrf_id`.
        # The default VRF is configured on the physical interface.
        # All interfaces use the same IP address on each end.
