...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;dhcp_server_interfaces</samp>](## "address_locking_settings.dhcp_server_interfaces") | List, items: String |  |  |  | The list of interfaces connected to the DHCP server.<br>When both `local_interface` and `dhcp_server_interfaces` are configured, EOS prioritizes `local_interface`.<br>Requires EOS version 4.36 or later. |
