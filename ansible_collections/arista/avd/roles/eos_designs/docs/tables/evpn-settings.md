...
=== "Table"
    ...
    | [<samp>evpn_prevent_readvertise_to_server_mode</samp>](## "evpn_prevent_readvertise_to_server_mode") | String |  | `as_path_acl` | Valid Values:<br>- <code>source_peer_asn</code><br>- <code>as_path_acl</code><br>- <code>peer_tag</code> | `evpn_prevent_readvertise_to_server_mode` controls the method of identifying EVPN routes that should not be advertised to the EVPN route-servers.<br>Only used when `evpn_prevent_readvertise_to_server` is set to `true`.<br>`source_peer_asn` mode configures an outbound route-maps towards EVPN route-servers which filter out BGP updates learned directly from the ASN of the route-servers. This mode will still allow routes learned via any other peer, even if they have the route-server's ASN in the AS-path.<br>`as_path_acl` mode configures an outbound route-maps and as-path access-lists which filter out BGP updates with the route-servers' ASN anywhere in the AS-path.<br>`peer_tag` method uses BGP Peer Tagging feature to filter out EVPN BGP routes labeled with the tag matching the tag of the route-server. Note that Arista EOS BGP Peer Tag is not a formal BGP Path attribute and is therefore not advertised to other BGP neighbors. This mode will still allow routes learned via any other peer (like an EVPN route learned over the per-VRF iBGP from the MLAG peer), even if they have the route-server's ASN in the AS-path. This method requires EOS 4.34.2F and newer.<br> |
...
=== "YAML"
    ...
    # `peer_tag` method uses BGP Peer Tagging feature to filter out EVPN BGP routes labeled with the tag matching the tag of the route-server. Note that Arista EOS BGP Peer Tag is not a formal BGP Path attribute and is therefore not advertised to other BGP neighbors. This mode will still allow routes learned via any other peer (like an EVPN route learned over the per-VRF iBGP from the MLAG peer), even if they have the route-server's ASN in the AS-path. This method requires EOS 4.34.2F and newer.
    evpn_prevent_readvertise_to_server_mode: <str; "source_peer_asn" | "as_path_acl" | "peer_tag"; default="as_path_acl">
