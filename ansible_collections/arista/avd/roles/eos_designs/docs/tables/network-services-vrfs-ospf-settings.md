...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;simple_auth_key</samp>](## "<network_services_keys.name>.[].vrfs.[].ospf.simple_auth_key") | String |  |  | Min Length: 1<br>Max Length: 8 | Password used with simple authentication.<br>Only plaintext passwords are supported here as `eos_designs` will encrypt the key for type7 for each individual `l3_interfaces`, `l3_port_channels` or `svis`.<br>To protect the password at rest it is strongly recommended to make use of Ansible Vault or similar.<br>Configuration at the interface level under `l3_interfaces` or `l3_port_channels` takes precedence.<br>`encrypt_passwords` value does not have any effect on this key. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;key</samp>](## "<network_services_keys.name>.[].vrfs.[].ospf.message_digest_keys.[].key") | String | Required |  | Min Length: 1<br>Max Length: 16 | Key password.<br>Only plaintext passwords are supported here as `eos_designs` will encrypt the password for each entry under `l3_interfaces`, `l3_port_channels` and `svis`.<br>To protect the password at rest it is strongly recommended to make use of a vault or similar.<br>Configuration at the interface level under `l3_interfaces` or `l3_port_channels` takes precedence.<br>`encrypt_passwords` value does not have any effect on this key. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;simple_auth_key</samp>](## "<network_services_keys.name>.[].vrfs.[].svis.[].nodes.[].ospf.simple_auth_key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;key</samp>](## "<network_services_keys.name>.[].vrfs.[].svis.[].nodes.[].ospf.message_digest_keys.[].key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;simple_auth_key</samp>](## "<network_services_keys.name>.[].vrfs.[].svis.[].ospf.simple_auth_key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;key</samp>](## "<network_services_keys.name>.[].vrfs.[].svis.[].ospf.message_digest_keys.[].key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;simple_auth_key</samp>](## "svi_profiles.[].nodes.[].ospf.simple_auth_key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;key</samp>](## "svi_profiles.[].nodes.[].ospf.message_digest_keys.[].key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;simple_auth_key</samp>](## "svi_profiles.[].ospf.simple_auth_key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password. |
    ...
    | [<samp>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;key</samp>](## "svi_profiles.[].ospf.message_digest_keys.[].key") | String |  |  |  | Key for OSPF simple authentication.<br>Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.<br>Maximum 8 characters for cleartext password. |
        ...
        vrfs:
          - name: <str; required; unique>
            ...
            ospf:
              ...
              # Only plaintext passwords are supported here as `eos_designs` will encrypt the key for type7 for each individual `l3_interfaces`, `l3_port_channels` or `svis`.
              ...
              simple_auth_key: <str; length 1-8>
              message_digest_keys: # 1-2 items
                - id: <int; required; unique>
                  hash_algorithm: <str; "md5" | "sha1" | "sha256" | "sha384" | "sha512"; default="sha512">

                  # Key password.
                  # Only plaintext passwords are supported here as `eos_designs` will encrypt the password for each entry under `l3_interfaces`, `l3_port_channels` and `svis`.
                ...
                nodes:
                  ...
                  - node: <str; required; unique>
                    ...
                    ospf:
                      ...
                      # Expects a cleartext password if `encrypt_passwords: true`, otherwise expects a type 7 password.
