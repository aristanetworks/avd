...
!!! warning "AVD 6.0.0 change of behavior"
    Starting from AVD version 6.0.0, using `eos_cli_config_gen` keys directly in `eos_designs` input data is no longer supported.
    These keys will be ignored during processing and warnings will be emitted during input validation.

    To disable the warnings, use the `avd_eos_designs_warn_eos_cli_config_gen_keys` role setting (default: `true`).
    See the [role settings documentation]('../tables/role-settings.md') for more information.

    **Always use `custom_structured_configuration` as described below to extend or override the configuration generated by `eos_designs`.**

## Overview

AVD `eos_designs` role flow is expected to work as follow:

1. from the input variables (inventory, group_vars, host_vars, ...) the `eos_designs` role generates the "structured_config". The `structured_config` and custom_structured_configuration are merged at the end of `eos_designs` processing to generate the final "structured_config" used by `eos_cli_config_gen`.
...
See [Connected Endpoints]('../tables/connected-endpoints.md')
...
See [Fabric Topology]('../tables/fabric-topology.md')
...
See [Network Services]('../tables/network-services.md')
...
The `override_radius_server` list will be `appended` to `radius_server` list without checking for element uniqueness, resulting in:
...
- User `192.168.42.10` is defined in all three data sources (`eos_designs` and `custom_structured_configuration` with prefixes `csc_1_` and `csc_2_`). Since the value of the primary key (`name`) matches in all 3 sources, the original data from `eos_designs` is first updated with data from `custom_structured_configuration` with prefix `csc_1_`. It is then updated again with data from `custom_structured_configuration` with prefix `csc_2_`, resulting in the priority for this name server being set to 3.
