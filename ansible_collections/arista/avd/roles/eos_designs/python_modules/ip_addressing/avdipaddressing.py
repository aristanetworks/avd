...
class AvdIpAddressing(AvdFacts, UtilsMixin):
    ...
    def _ip(self, pool: str, prefixlen: int, subnet_offset: int, ip_offset: int, add_subnet_space: bool = True) -> str:
        ...
        subnet = ipaddress.ip_network((int(pool_network.network_address) + subnet_offset * (subnet_size if add_subnet_space else 1), prefixlen))
        try:
            ip = subnet[ip_offset]
