...
class UtilsMixin:
    ...
    @cached_property
    def _default_policy_path_groups(self) -> list:
        """
        ...
        if not path_groups:
            # all path-groups are excluded from default-policy
            raise AristaAvdError(
                "Empty list of path-groups to use for the AVD autogenerated default policy, either `wan_path_groups` is empty "
                "or all path-groups have `excluded_from_default_policy` set tot `true`. Make sure that at least one path-group can be "
                "configured for the default-policy"
            )
        elif not [path_group for path_group in path_groups if path_group in local_path_group_names]:
            # No common path-group between this device local path-groups and the available path-group for the default policy
            raise AristaAvdError(
                f"None of this device local path-groups {local_path_group_names} is eligible to be included in the "
                "AVD default-policy. Make sure that at least one path-group for the device is not configured with "
                "`excluded_from_default_policy` set tot `true` under `wan_path_groups`."
            )
        return path_groups
    ...
    def _default_control_plane_virtual_topology(self) -> dict:
        ...
        """
        path_groups = self._default_policy_path_groups
    ...
    def _default_wan_policy(self) -> dict:
        ...
        return {
            ...
            "default_virtual_topology": {"path_groups": [{"names": self._default_policy_path_groups}]},
        }
