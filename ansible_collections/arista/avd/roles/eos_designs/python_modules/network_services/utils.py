...
class UtilsMixin:
    ...
    def _default_policy_path_group_names(self) -> list:
        ...
        # TODO refactor once the shared_utils option is present
        local_path_group_names = {path_group["name"] for path_group in self.shared_utils.wan_local_path_groups}
        if not path_group_names.intersection(local_path_group_names):
            # No common path-group between this device local path-groups and the available path-group for the default policy
            raise AristaAvdError(
                f"Unable to generate the default WAN policy as none of the device local path-groups {local_path_group_names} "
