...
1. from the input variables (inventory, group_vars, host_vars,...), a task first sets ansible facts for each device
2. the facts are then used in a subsequent task to generate a structured_config
3. this structured_config is what is then used by `eos_cli_config_gen` role to generate the intended config

A caveat is that it is possible to define in the inventory, group_vars, host_vars,... some variables that are
used by `eos_designs` and others used by `eos_cli_config_gen` roles. And while it is perfectly acceptable,
...
The consequence is that upgrading AVD to a newer version for instance, or configuring additional `eos_designs` functionnality in some existing AVD repo,
could lead to some additional variable keys being generated by `eos_designs` in the structured_config, overwriting previously working `eos_cli_config_gen` keys.
...
The recommendation is to use custom_structured_configuration as described in [the following section of this guide](./custom-structured-configuration.md). This configuration will always prevails over the `eos_designs` generated variables.
