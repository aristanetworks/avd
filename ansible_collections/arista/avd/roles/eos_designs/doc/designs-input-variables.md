...
The fabric topology variables define the fabric nodes, connectivity between them as well as foundational configuration elements to configure the underlay and overlay switch configuration.
...
!!! tip
    The node type setting can be automatically derived from a switch name by defining the pattenrs in the `default_node_type` data model.

As explained above, you can defined your own types of devices. AVD only provides default node types.
...
!!! tip
    Leverage default interfaces data model to auto define uplink and downlink interfaces based on the node id.
...
#### Node type EVPN services configuration
...
- The keys used to define `connected_endpoints` are configurable using [`connected_endpoints_keys`](#connected-endpoints-keys-settings).
