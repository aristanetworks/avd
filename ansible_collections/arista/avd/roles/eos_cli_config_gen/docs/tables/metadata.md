...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;validate_no_errors_period</samp>](## "metadata.validate_no_errors_period") | Integer |  |  |  | Threshold (in days) defining how far back to check the logging buffer for error-level logs during the validation performed by the `anta_runner` role. |
...
=== "YAML"
    ...
    metadata:
      ...
      digital_twin:
        ...
        # Management IPv4_address/Mask assigned to a replica of the fabric device within the Digital Twin environment.
        ip_addr: <str>
      ...
      # Threshold (in days) defining how far back to check the logging buffer for error-level logs during the validation performed by the `anta_runner` role.
