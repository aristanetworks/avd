...
| [**VerifyLLDPNeighbors**](https://anta.arista.com/stable/api/tests/connectivity/#anta.tests.connectivity.VerifyLLDPNeighbors){:target="_blank"} | Verifies LLDP neighbors for Ethernet interfaces. | <ul><li>`ethernet_interfaces[]`</li><li>`ethernet_interfaces[].metadata.validate_state`</li><li>`ethernet_interfaces[].metadata.validate_lldp`</li><li>`ethernet_interfaces[].metadata.peer`</li><li>`ethernet_interfaces[].metadata.peer_interface`</li><li>`peer.metadata.is_deployed`</li><li>`peer.ethernet_interfaces[].shutdown`</li><li>`peer.metadata.dns_domain`</li></ul> |
