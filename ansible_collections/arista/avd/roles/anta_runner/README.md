...
The `anta_runner` role utilizes Ansible's standard verbosity flags (`-v`, `-vv`, etc.) to control log output from its various components, including PyAVD, ANTA, underlying libraries, and the `anta_workflow` action plugin. Adjust the verbosity level using the `ansible-playbook` command:

- **Default (no `-v` flag):** `WARNING` level and above for all sources
- `-v`: `INFO` level for all sources, except `httpx` logs (which remain at `WARNING` due to high chattiness)
- `-vv`: `INFO` level for all sources, including `httpx`
- `-vvv`: `DEBUG` level for all sources except ANTA **and** its low-level libraries
- `-vvvv`: `DEBUG` level for all sources except ANTA low-level libraries
- `-vvvvv`: `DEBUG` level for all sources

!!! tip
    When troubleshooting why specific AVD-generated tests might be missing from a device's final catalog, running `ansible-playbook` with `-vvv` is particularly helpful. PyAVD will output detailed information about skipped tests and the reasons for their exclusion.
...
Console logs adhere to standard Ansible logging configurations. For persistent storage and easier analysis, especially with high verbosity, consider configuring `log_path` in your `ansible.cfg` file. This redirects the primary output stream to a specified log file, which is very useful for troubleshooting. For more details, refer to the [Ansible Logging Documentation](https://docs.ansible.com/ansible/latest/reference_appendices/logging.html).

### Task Failure Conditions

The `anta_runner` role will mark the Ansible task as failed if any of the following conditions occur:

- **No tests executed:** If no tests were run against any device (and it's not a `dry_run`)
- **Execution errors:** If errors arise during the role's execution, originating from Ansible, PyAVD, or ANTA
- **Test failures or errors:** If any ANTA test reports a failure or error status.

Understanding these conditions can help you quickly identify the cause of a failed playbook run.
