...
The `anta_runner` utilizes Ansible verbosity flags (`-v`, `-vv`, `-vvv`) to control log output levels for both the console and detailed log files, aiding in monitoring execution and diagnosing issues.

### Console Logging

- **Plugin/Workflow Logs:** Displays logs from the plugin itself and PyAVD based on verbosity: `WARNING` and above (default), `INFO` (`-v`), or `DEBUG` (`-vvv`).
- **ANTA Libraries Logs:** Logs from ANTA and its underlying libraries are filtered; only messages at `WARNING` level or higher are shown on the console.
- **Unique ID Prefix:** Each console log message is prefixed with a unique ID (e.g., `anta-workflow` or `anta-run-xxxxxxxx`) to identify which part of the execution it came from (main process or a specific run). This helps correlate console messages to specific processes or batches.
- **Task Failure:** Log messages at `ERROR` level or higher appearing on the console will cause the Ansible task to fail.
- **AVD Skipped Tests:** Note that using `-vvv` *also* displays specific information about tests skipped from AVD-generated catalogs directly on the console.

!!! note
    The output directed to the Ansible console (described above) adheres to standard Ansible logging configurations. For instance, you can set `log_path` in your `ansible.cfg` to redirect this primary output stream to a central log file managed by Ansible. For more details on Ansible's logging capabilities, refer to the [Ansible Logging Documentation](https://docs.ansible.com/ansible/latest/reference_appendices/logging.html).
