...
| [**VerifyBGPPeerSession**](https://anta.arista.com/stable/api/tests/routing.bgp/#anta.tests.routing.bgp.VerifyBGPPeerSession){:target="_blank"} | Verifies that BGP peer sessions are in an 'established' state. | <ul><li>`router_bgp.neighbors`</li><li>`router_bgp.vrfs.neighbors.metadata.validate_state`</li></ul> |
