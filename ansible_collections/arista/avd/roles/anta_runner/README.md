...
Devices with `is_deployed: false` set as part of AVD Design inputs will automatically be ignored. See [Device Deployment Status](#device-deployment-status) for more details.
...
The `anta_runner` role leverages structured configuration files generated by the `eos_designs` role to create ANTA test catalogs. Several `eos_designs` input keys directly influence which tests are generated and how they behave.
