...
With AVD version 5.0.0 the default interface descriptions are changed to give better consistency. Detailed changes are described in the sub-sections below.

To revert all descriptions to 4.x syntax follow the following steps:

1. Add the following custom description template variables to relevant group variable(s):
...
`endpoint_port_channel` for *all* `<connected_endpoints_keys.key>` adapters. Also the default description template must be updated.
