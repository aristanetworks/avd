...
In AVD 4.0.0, setting `enabled: true` under `router_ospf.redistribute.bgp/connected/static` was not
...
As of AVD 5.0.0, the `enabled` keyword is required under `router_ospf.redistribute.bgp/connected/static`:
