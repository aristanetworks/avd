...
### Use tags to match routes to redistribute in WAN for VRF default

With AVD 5.0.0, for WAN designs, the mechanism used to match routes to be
injected in EVPN for VRF default is changed from SOO to tags.

To retain the previous configuration `wan_use_soo_for_route_injection: true`

```diff
+ wan_use_soo_for_route_injection: true
```
...
To retain the previous configuration use `use_port_channel_for_direct_ha: false` under `wan_ha`.
