...
As of AVD 5.0.0, the configuration of `flags` in the `UDP` protocol under traffic policies is now generated, and the configuration for `source/destination port` and `field set` can be generated together for the `TCP/UDP` protocol. Also, the configuration order has been fixed as per the EOS running-config.
