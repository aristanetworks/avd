...
AVD versions below 5.0.0 has `redistribute igmp` configured for a BGP svi/vlan belonging to a VRF with EVPN multicast, but having `redistribute igmp` configured for a svi/vlan belonging to a VRF with evpn multicast could cause conflicts in the control-plane.

In AVD version 5.0.0 `redistribute igmp` is no longer created for BGP svi/vlan belonging to a VRF with EVPN multicast.

The `redistribute igmp` config can be retained by setting `redistribute_igmp_vrf: true` (default value is false) under the `tenants[].evpn_l2_multicast`.
