...
```yaml hl_lines="6 8 10" title="5.x"
router_ospf:
  process_ids:
    - id: 400
      redistribute:
...
-     vlan: 35
-     trunk: tagged
-   vlan_translations:#### Fix issues with flags, source/destination ports, and field set for TCP/UDP protocol under traffic-policies
...
As of AVD 5.0.0, the configuration of `flags` in the `UDP` protocol under traffic policies is no longer generated, and the configuration for `source/destination port` and `field set` can be generated together for the `TCP/UDP` protocol. Also, the configuration order has been fixed as per the EOS running-config.
+       native_vlan: 20
+       native_vlan_tag: true
+       groups:
+         - group1
