...
### Changes in MTU values and support for `p2p_uplinks_mtu` in platform settings

#### Default MTU value changed from `9214` to `9194` for Dps1 and WAN HA interfaces

In AVD 4.0.0, the default MTU for Dps1, LAN Uplink interfaces, and WAN HA interfaces was `9214`.
In AVD 5.0.0, the default MTU for Dps1 and WAN HA interfaces has been changed to `9194`.

#### New Feature: `p2p_uplinks_mtu` support in platform settings

In AVD 5.0.0, support for `p2p_uplinks_mtu` has been added in platform settings. The precedence for setting MTU on point-to-point uplink interfaces is now as follows:

1. `node_type_uplink_mtu`
2. `platform_p2p_uplinks_mtu`
3. `p2p_uplinks_mtu`
4. Default value: `9214`

MTU on WAN HA interfaces
...
MTU on point to point uplink interfaces via platform settings
