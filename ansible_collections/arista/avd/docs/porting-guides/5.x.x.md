...
In AVD 4.0.0, the tables for `IPv4` and `IPv6` in ethernet and pory-channel interfaces were rendered only if `ethernet_interfaces[].type` or `port_channel_interfaces[].type` was set to `routed/l3dot1q`.
