...
The interface description for the L2 uplink/downlink interfaceshas changed to `L2_<peer>_<peer_interface>`. Previously the description was `<peer | upper>_<peer_interface>`.
...
To retain the prevoius MTU on the WAN HA interfaces, set `<node_type>.defaults/node_groups[]/node_groups[].nodes[]/nodes[].wan_ha.mtu` as `9214`.
...
required, and it was possible to render the follwing:
