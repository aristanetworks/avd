...
| `arista.avd.eos_designs` | `avd_eos_designs_facts` | `true` | Generate `avd_switch_facts` which is a prerequisite for `eos_designs_structured_config` and `eos_designs_documentation`. |
| `arista.avd.eos_designs` | `avd_eos_designs_debug` | `false` | Dump all vars and facts per host after generating `avd_switch_facts`. |
| `arista.avd.eos_designs` | `avd_eos_designs_structured_config` | `true` | Generate structured configuration per device. |
| `arista.avd.eos_designs` | `eos_designs_documentation.enabled` | `true` | Generate fabric MarkDown documentation. |
...
| `arista.avd.eos_designs` | `avd_eos_designs_unset_facts` | `true` | Unset `avd_switch_facts` to gain a small performance improvement since Ansible needs to handle less variables. |
| `arista.avd.eos_cli_config_gen` | `avd_eos_cli_config_gen_config` | `true` | Generate device EOS configurations. |
| `arista.avd.eos_cli_config_gen` | `generate_device_documentation` | `true` | Generate device MarkDown documentation. |
