...
### Added valid source protocol values for Router BGP redistribute route in `address_family_ipv4_multicast`, `address_family_ipv6`, and `vrf` configurations

In AVD 4.0.0 we had a list of str as source protocol for `address_family_ipv4_multicast`, `address_family_ipv6`, and `vrf` under router BGP.

As of AVD 5.0.0 a list of str as source protocol for `address_family_ipv4_multicast`, `address_family_ipv6`, and `vrf` under router BGP updated with valid values.
