...
In AVD version 5.0.0, `vrf_id` is used instead of `vrf_vni` to calculate the offset for IP address of EVPN underlay multicast group for a VRF when `evpn_l3_multicast` is enabled. The configurations will only change in case `vrf_id` and `vrf_vni` are set to different values.

To retain the previous configuration, either exclude the `vrf_id` from the input or ensure that `vrf_id` is identical to `vrf_vni`.
