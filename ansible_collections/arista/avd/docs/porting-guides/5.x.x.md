...
### Raise the AVD error if `sFlow` is enabled on an interface but `sflow_settings.destinations` is not defined

In AVD 4.0.0, if `sFlow` was enabled on an interface but `sflow_settings.destinations` was not defined, the sFlow configuration was not generated.

As of AVD 5.0.0, if `sFlow` is enabled on an interface but `sflow_settings.destinations` is not defined, the sFlow configuration is not generated and an AVD error is raised.
