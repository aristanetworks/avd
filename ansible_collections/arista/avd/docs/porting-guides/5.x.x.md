...
### Default value of `shutdown_bgp_towards_undeployed_peers` and `shutdown_interfaces_towards_undeployed_peers` is changed to `true`
...
To configure `no shutdown`, set the value to `false`:
