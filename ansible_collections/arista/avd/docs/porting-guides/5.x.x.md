...
### `router_ospf.redistribute.x.enabled` is required

In AVD 4.0.0, setting `enabled: true` under `router_ospf.redistribute.x` was not
...
As of AVD 5.0.0, the `enabled` keyword is required under `router_ospf.redistribute.x`:
