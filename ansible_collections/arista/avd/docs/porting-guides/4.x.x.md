...
To keep existing EOS default isis circuit-type set:
...
- `l3_edge.p2p_links.[].qos_profile` default changed from `null` to now leverage `p2p_uplinks_qos_profile` as it's default value.
...
- `l3_edge.p2p_links.[].include_in_underlay_protocol` default changed from `fasle` to `true`.
