...
A new key, `bgp_update_wait_install: <bool> -> default true` was introduced to implement the best practice of enable router bgp `update wait-install` in default and per vrf when the vrf contains configured neighbors.
