...
| <samp>pattern_keys</samp> | Dictionary | | | Pattern: `<regex>` | Dictionary in the format `{<regex>: {<schema>}}`.<br>`regex` is a regex string used for matching keys in the data. `schema` is the schema used for validating those keys. This is a recursive schema, so the value must conform to AVD Schema<br>Regular expressions are anchored, meaning they must match the full key name. |
