...
Starting AVD 5.0.0, `vrf_vni` will be replaced with `vrf_id` in the algorithm to to set the offset for ip address of evpn underlay multicast group when evpn_l3_multicast is enabled.

See the [Porting guide for AVD 5.x.x](../porting-guides/5.x.x.md#vrf_id-will-be-used-instead-of-vrf_vni-to-get-the-ip-address-of-evpn-underlay-multicast-group)
