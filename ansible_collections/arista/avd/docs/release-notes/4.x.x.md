...
As part of optimizing performance, the amount of `switch.*` facts has been limited.
The `switch.*` and `avd_switch_facts` are internal datamodels, that are used for sharing data between various stages of `eos_designs`.

It should be avoided to use these facts in custom templates for interface descriptions or IP addressing, since they are subject to change
without notice.
