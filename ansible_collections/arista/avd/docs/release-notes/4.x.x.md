...
- AVD now replaces sensitive data (passwords, keys, ...) in device documentation by the string `<removed>` by default. To disable this behavior, see the [Overview](../roles/eos_cli_config_gen/README.md) for `eos_cli_config_gen` role.
