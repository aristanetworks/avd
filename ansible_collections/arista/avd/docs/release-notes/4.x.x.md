...
#### Breaking Changes in eos_cli_config_gen

To avoid ambiguous YAML data input, the data model for `queue_monitor_length` has been updated to require the `enabled: true` key to be set.
...
See details in the [Porting guide for AVD 4.x.x](../porting-guides/4.x.x.md#todo).
