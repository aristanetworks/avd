...
collection to perform actions on the CloudVision platform. These modules are currently available in `v1` and `v3`, but `v1` modules
will soon be deprecated and later removed.
...
`v1` modules can still be used by setting `cv_collection: v1`.
