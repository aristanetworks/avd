...
### Deprecated data models for `peer_groups` options under `router_bgp.vrfs[].address_family_*`

The data models for `peer_groups` options under `router_bgp.vrfs[].address_family_*` dicts have been deprecated because the corresponding configuration does not exist in EOS. Any configuration applied to EOS under the corresponding VRF->AF config submode get automatically moved out to the global bgp context.

See details in the [Porting guide for AVD 4.x.x](../porting-guides/4.x.x.md#deprecated-data-models-for-peer_groups-options-under-router_bgpvrfsaddress_family_*)
