...
from ansible_collections.arista.avd.plugins.plugin_utils.utils import AvdSwitchFactsDefaultDict, DeprecatedDict, get_templar, write_file
...
class ActionModule(ActionBase):
    def run(self, tmp: Any = None, task_vars: dict | None = None) -> dict:
        ...
        if self._templar.is_template(task_vars["switch"]):
            ...
            try:
                task_vars["switch"] = self._templar.template(task_vars["switch"], fail_on_undefined=False)
            except Exception as e:
                msg = "Exception during templating of task_var 'switch'"
                raise AnsibleActionFail(msg) from e

        task_vars["switch"] = DeprecatedDict(
            task_vars["switch"],
            _display=self._display,
            _message=(
                "Inline jinja templating based on 'switch.*' facts generated by 'eos_designs' is deprecated. "
                "Open a GitHub discussion so we can find an alternative."
            ),
        )

        # Read ansible variables and perform templating to support inline jinja2
        for var in task_vars:
            if str(var).startswith(("ansible", "molecule", "hostvars", "vars", "avd_switch_facts", "switch")):
                continue
