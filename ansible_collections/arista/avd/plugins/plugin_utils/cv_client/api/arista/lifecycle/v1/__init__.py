...
class DeviceLifecycleSummaryKey(betterproto.Message):
    """
...
@dataclass(eq=False, repr=False)
class SoftwareEol(betterproto.Message):
    ...
    end_of_support: datetime = betterproto.message_field(2)
    """end_of_support of a SoftwareEOL"""


@dataclass(eq=False, repr=False)
class DateAndModels(betterproto.Message):
    """
    DateAndModels has an "end of" date along with the models that has this
    exact "end of" date
    """
    ...
    models: "___fmp__.MapStringInt32" = betterproto.message_field(2)
    """models with this exact "end of" date mapped to its count"""


@dataclass(eq=False, repr=False)
class HardwareLifecycleSummary(betterproto.Message):
    """HardwareLifecycleSummary represents a hardware lifecycle summary"""

    end_of_life: "DateAndModels" = betterproto.message_field(1)
    """end_of_life of a HardwareLifecycleSummary"""

    end_of_sale: "DateAndModels" = betterproto.message_field(2)
    """end_of_sale of a HardwareLifecycleSummary"""

    end_of_tac_support: "DateAndModels" = betterproto.message_field(3)
    """end_of_tac_support of a HardwareLifecycleSummary"""

    end_of_hardware_rma_requests: "DateAndModels" = betterproto.message_field(4)
    """end_of_hardware_rma_requests of a HardwareLifecycleSummary"""


@dataclass(eq=False, repr=False)
class DeviceLifecycleSummary(betterproto.Message):
    """
    DeviceLifecycleSummary is the state model that represents the lifecycle
    summary of a device
    """

    key: "DeviceLifecycleSummaryKey" = betterproto.message_field(1)
    """DeviceLifecycleSummaryKey is the key of DeviceLifecycleSummary"""

    software_eol: "SoftwareEol" = betterproto.message_field(2)
    """software_eol is the software end of life of a device"""

    hardware_lifecycle_summary: "HardwareLifecycleSummary" = betterproto.message_field(
        3
    )
    """
...
@dataclass(eq=False, repr=False)
class DeviceLifecycleSummaryRequest(betterproto.Message):
    key: "DeviceLifecycleSummaryKey" = betterproto.message_field(1)
    """
    Key uniquely identifies a DeviceLifecycleSummary instance to retrieve. This
...
@dataclass(eq=False, repr=False)
class DeviceLifecycleSummaryResponse(betterproto.Message):
    value: "DeviceLifecycleSummary" = betterproto.message_field(1)
    """
...
@dataclass(eq=False, repr=False)
class DeviceLifecycleSummaryStreamRequest(betterproto.Message):
    partial_eq_filter: List["DeviceLifecycleSummary"] = betterproto.message_field(1)
    """
    ...
    end: Returns the state of each DeviceLifecycleSummary at end.     * Each
    DeviceLifecycleSummary response is fully-specified (all fields set).   *
    start: Returns the state of each DeviceLifecycleSummary at start, followed
    by updates until now.     * Each DeviceLifecycleSummary response at start
    is fully-specified, but updates may be partial.   * start and end: Returns
    the state of each DeviceLifecycleSummary at start, followed by updates
    until end.     * Each DeviceLifecycleSummary response at start is fully-
...
@dataclass(eq=False, repr=False)
class DeviceLifecycleSummaryStreamResponse(betterproto.Message):
    value: "DeviceLifecycleSummary" = betterproto.message_field(1)
    """
    ...
    time: datetime = betterproto.message_field(2)
    """
    Time holds the timestamp of this DeviceLifecycleSummary's last
    modification.
    """

    type: "__subscriptions__.Operation" = betterproto.enum_field(3)
    """
    Operation indicates how the DeviceLifecycleSummary value in this response
...
class DeviceLifecycleSummaryServiceStub(betterproto.ServiceStub):
    async def get_one(
        self,
...
class DeviceLifecycleSummaryServiceBase(ServiceBase):
    ...
    async def get_all(
        ...
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
        yield DeviceLifecycleSummaryStreamResponse()
