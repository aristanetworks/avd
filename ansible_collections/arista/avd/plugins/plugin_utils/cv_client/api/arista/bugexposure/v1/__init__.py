...
class Acknowledgement(betterproto.Enum):
    """
    Acknowledgement is an enumeration key for a BugExposure model that defines
    the acknowledgement state for the computed bugs
    """
...
class HighestExposure(betterproto.Enum):
    """
    HighestExposure is an enumeration that defines the options for highest
    exposure
    """

    HIGHEST_EXPOSURE_UNSPECIFIED = 0
    """
    If not given this will be the default value and it will compute devices
    with any highest exposure
    """
...
class BugExposureKey(betterproto.Message):
    ...
    acknowledgement: "Acknowledgement" = betterproto.enum_field(2)
    """acknowledgement is one of the options for Acknowledgement enum"""


@dataclass(eq=False, repr=False)
class BugExposure(betterproto.Message):
    """
    BugExposure is the state model that represents the exposure a device has to
    bugs
    """

    key: "BugExposureKey" = betterproto.message_field(1)
    """BugExposureKey is the key of BugExposure"""

    bug_ids: "___fmp__.RepeatedInt32" = betterproto.message_field(2)
    """bug_ids is a list of bug alerts affecting the device with type Bug"""

    cve_ids: "___fmp__.RepeatedInt32" = betterproto.message_field(3)
    """cve_ids is a list of bug alerts affecting the device with type CVE"""

    bug_count: Optional[int] = betterproto.message_field(
        4, wraps=betterproto.TYPE_INT32
    )
    """bug_count is the number of bug alerts with type Bug"""

    cve_count: Optional[int] = betterproto.message_field(
        5, wraps=betterproto.TYPE_INT32
    )
    """cve_count is the number of bug alerts with type CVE"""

    highest_bug_exposure: "HighestExposure" = betterproto.enum_field(6)
    """highest_bug_exposure is the highest exposure with type Bug"""

    highest_cve_exposure: "HighestExposure" = betterproto.enum_field(7)
    """highest_cve_exposure is the highest exposure with type CVE"""


@dataclass(eq=False, repr=False)
class MetaResponse(betterproto.Message):
    time: datetime = betterproto.message_field(1)
    """
    Time holds the timestamp of the last item included in the metadata
    calculation.
    """

    type: "__subscriptions__.Operation" = betterproto.enum_field(2)
    """
    Operation indicates how the value in this response should be considered.
    Under non-subscribe requests, this value should always be INITIAL. In a
    subscription, once all initial data is streamed and the client begins to
    receive modification updates, you should not see INITIAL again.
    """
...
@dataclass(eq=False, repr=False)
class BugExposureRequest(betterproto.Message):
    key: "BugExposureKey" = betterproto.message_field(1)
    """
    Key uniquely identifies a BugExposure instance to retrieve. This value must
    be populated.
    """
...
@dataclass(eq=False, repr=False)
class BugExposureResponse(betterproto.Message):
    value: "BugExposure" = betterproto.message_field(1)
    """
...
@dataclass(eq=False, repr=False)
class BugExposureStreamRequest(betterproto.Message):
    partial_eq_filter: List["BugExposure"] = betterproto.message_field(1)
    """
    ...
    end: Returns the state of each BugExposure at end.     * Each BugExposure
    response is fully-specified (all fields set).   * start: Returns the state
    of each BugExposure at start, followed by updates until now.     * Each
    BugExposure response at start is fully-specified, but updates may be
    partial.   * start and end: Returns the state of each BugExposure at start,
    followed by updates     until end.     * Each BugExposure response at start
...
@dataclass(eq=False, repr=False)
class BugExposureStreamResponse(betterproto.Message):
    value: "BugExposure" = betterproto.message_field(1)
    """
    ...
    """
    Operation indicates how the BugExposure value in this response should be
...
class BugExposureServiceStub(betterproto.ServiceStub):
    async def get_one(
        self,
...
class BugExposureServiceBase(ServiceBase):
    ...
    async def get_all(
        ...
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
        yield BugExposureStreamResponse()
