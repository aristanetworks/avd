...
class AssignmentKey(betterproto.Message):
    """AssignmentKey allows to uniquely identify an assignment."""

    system_id: Optional[str] = betterproto.message_field(
...
class Assignment(betterproto.Message):
    ...
    key: "AssignmentKey" = betterproto.message_field(1)
    """key uniquely identifies the assignment of system_id to the cluster."""

    clusters: "Clusters" = betterproto.message_field(2)
...
class Clusters(betterproto.Message):
    """
    Clusters wraps a cluster list which contain the information about the
    hosts.
    """

    values: List["Cluster"] = betterproto.message_field(2)
...
class Cluster(betterproto.Message):
    ...
    """
    name of the cluster. The name can change over time as new clusters are
    added or removed.
    """

    hosts: "___fmp__.RepeatedString" = betterproto.message_field(2)
    """hosts in the cluster that the devices can connect to."""


@dataclass(eq=False, repr=False)
class AssignmentRequest(betterproto.Message):
    key: "AssignmentKey" = betterproto.message_field(1)
    """
    Key uniquely identifies a Assignment instance to retrieve. This value must
    be populated.
    """
...
@dataclass(eq=False, repr=False)
class AssignmentResponse(betterproto.Message):
    value: "Assignment" = betterproto.message_field(1)
    """
    ...
    time: datetime = betterproto.message_field(2)
    """
    Time carries the (UTC) timestamp of the last-modification of the Assignment
    instance in this response.
    """


@dataclass(eq=False, repr=False)
class AssignmentStreamRequest(betterproto.Message):
    partial_eq_filter: List["Assignment"] = betterproto.message_field(1)
    """
...
@dataclass(eq=False, repr=False)
class AssignmentStreamResponse(betterproto.Message):
    value: "Assignment" = betterproto.message_field(1)
    """
    ...
    """
    Operation indicates how the Assignment value in this response should be
...
class AssignmentServiceStub(betterproto.ServiceStub):
    async def get_one(
        self,
...
class AssignmentServiceBase(ServiceBase):
    ...
    async def get_all(
        ...
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
        yield AssignmentStreamResponse()
