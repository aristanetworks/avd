...
def create_node_dict(file_data, filename):
    ...
    if file_data and 'ethernet_interfaces' in list(file_data.keys()) and file_data['ethernet_interfaces']:
...
def find_root_nodes(data, root_dict=None):
    """
    Find a root node
    ...
    if "nodes" in list(data.keys()) and data["nodes"]:
        for node in data["nodes"]:
            ...
            root_dict["neighbours"].append(neighbour_dict)
        return root_dict

    if "groups" in list(data.keys()) and data["groups"]:
        for group in data["groups"]:
            root_dict = find_root_nodes(group, root_dict)
        return root_dict
...
def create_graph_dict(output_list, nodes=None, node_neighbour_dict=None):
    ...
    for group in output_list:
        ...
        if 'groups' in list(group.keys()) and group["groups"]:
