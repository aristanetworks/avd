...
def draw_groups_recursive(d, ld, ol, x, y, lum):
    ...
    if "psuedo" not in ol:
        ...
        d.append(
            draw.Rectangle(
                x,
                y - (((ol["height"] - 1) * LVLSPACING)) - GROUPOFFSET - HEIGHTMARGIN + BOXMARGIN,
                ol["width"],
                (((ol["height"] - 1) * LVLSPACING)) + GROUPOFFSET + HEIGHTMARGIN,
                fill=fill,
                rx="25",
                stroke=f"hsl(0, 0%, {lum * .75}%)",
                stroke_width=4,
            )
        )
        textlength = calculate_text_length(ol["name"], TITLEFONTSIZE)
        titles.append(
            draw.Rectangle(x + ol["width"] / 2 - textlength / 2, y - TITLEFONTSIZE + TITLEOFFSET + BOXMARGIN - 3, textlength, TITLEFONTSIZE - 1, fill=fill)
        )
        titles.append(
            draw.Text(ol["name"], TITLEFONTSIZE, x + ol["width"] / 2, y - TITLEFONTSIZE + BOXMARGIN + TITLEOFFSET, fill="black", text_anchor="middle")
        )
    ...
    for idx, child in enumerate(ol["nodes"]):
        ...
        d.append(draw.Rectangle(child["x"] - (ROUTERSIZE / 2), child["y"] - (ROUTERSIZE / 2), ROUTERSIZE, ROUTERSIZE, fill="#5167B7", text=child["name"]))
        ...
        if node_val not in undefined_rank_nodes:
            for i in node_details:
                ...
                if node_level_dict[node_val] < node_level_dict[i["neighborDevice"]]:
                    ...
                    if (
                        i["neighborPort"] not in node_port_val[i["neighborDevice"]]["top"]
                        and i["neighborPort"]
                        and i["neighborPort"] not in node_port_val[i["neighborDevice"]]["checked"]
                    ):
                ...
                if node_level_dict[node_val] > node_level_dict[i["neighborDevice"]]:
                    ...
                    if (
                        i["neighborPort"] not in node_port_val[i["neighborDevice"]]["bottom"]
                        and i["neighborPort"]
                        and i["neighborPort"] not in node_port_val[i["neighborDevice"]]["checked"]
                    ):
            ...
            for j in range(i + 1, len(level_list)):
                for node_detail in graph_dict[level_list[i]]:
                    if node_detail["neighborDevice"] == level_list[j]:
                        ...
                        if (node_detail["neighborPort"] not in node_port_val[level_list[j]]["left"]) and (
                            node_detail["neighborPort"] not in node_port_val[level_list[j]]["checked"]
                        ):
