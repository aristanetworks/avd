...
class PlatformMixin:
    ...
    def default_interfaces(self: SharedUtils) -> dict:
        ...
        for default_interface in default_interfaces:
            for platform in default_interface.get("platforms", []):
                if search(f"^{platform}$", device_platform) and (
                    self.type in default_interface.get("types", []) or "default" in default_interface.get("types", [])
                ):
        ...
        for default_interface in default_interfaces:
            for platform in default_interface.get("platforms", []):
                if search(f"^{platform}$", "default") and (self.type in default_interface.get("types", []) or "default" in default_interface.get("types", [])):
                    return default_interface
