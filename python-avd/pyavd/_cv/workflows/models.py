...
@dataclass(frozen=True)
class AvdConfigletRef:
    """Reference to a configlet by name, used within AvdContainer."""
...
class AvdContainer:
    ...
    configlets: tuple[AvdConfigletRef, ...] = field(default_factory=tuple)
    ...
    def from_dict(cls, data: dict[str, Any]) -> AvdContainer:
        ...
        try:
            ...
            configlets = tuple(AvdConfigletRef(name=item["name"]) for item in configlets_data)
...
@dataclass(frozen=True)
class CVConfigletID:
    """CloudVision configlet ID based on name with cached UUID."""

    name: str

    @cached_property
    def value(self) -> str:
        return f"{AVD_ENTITY_PREFIX}{uuid5(AVD_NAMESPACE, self.name)}"


@dataclass(frozen=True)
class CVContainerID:
    """CloudVision container ID based on hierarchy path with cached UUID."""

    path: tuple[str, ...]

    @cached_property
    def value(self) -> str:
        return f"{AVD_ENTITY_PREFIX}{uuid5(AVD_NAMESPACE, '/'.join(self.path))}"
...
class CVManifest:
    ...
    def from_avd_manifest(cls, avd_manifest: AvdManifest) -> CVManifest:
        ...
        for avd_configlet in avd_manifest.configlets:
            cv_configlet = CVConfiglet(
                avd_configlet=avd_configlet, id=CVConfigletID(name=avd_configlet.name), description="Configlet created and uploaded by AVD."
            )
    ...
    def _process_container_recursively(
        ...
        configlet_ids = []
        for configlet_ref in container.configlets:
            if configlet_ref.name not in cv_configlet_map:
                msg = f"Configlet '{configlet_ref.name}' is assigned to a container but is not found in the input definition."
                raise CVManifestError(msg)
            configlet_ids.append(cv_configlet_map[configlet_ref.name].id)

        # Create the parent CVContainer object.
        cv_container = CVContainer(
            avd_container=container,
            id=CVContainerID(path=tuple(current_path.split("/"))),
            is_root=(parent_path == ""),
...
class CVConfiglet:
    ...
    def api_tuple(self) -> tuple[str, str, str, str]:
        ...
        return (self.id.value, self.name, self.description, self.file)
...
class CVContainer:
    ...
    id: CVContainerID
    is_root: bool
    configlet_ids: tuple[CVConfigletID, ...] = field(default_factory=tuple)
    child_ids: tuple[CVContainerID, ...] = field(default_factory=tuple)
    ...
    def api_tuple(self) -> tuple[Any, ...]:
        ...
        return (
            ...
            self.name,
            self.description or "",
            [configlet_id.value for configlet_id in self.configlet_ids],
            self.tag_query,
            [child_id.value for child_id in self.child_ids],
            self.match_policy,
        )
