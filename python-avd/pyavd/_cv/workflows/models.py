...
class DeployToCvResult:
    ...
    deployed_configs2: dict[str, AvdEosConfig] = field(default_factory=dict)
...
class AvdDevice:
    ...
    pathfinder_metadata: AvdPathfinderMetadata
    hostname: str
    """
    Device hostname or intended hostname.
    `serial_number` or `system_mac_address` must be set if the hostname is not already configured on the device or
...
class InternalDevice:
    """Internal representation of a device for deployment with inputs from AVD along with the result."""

    avd_device: AvdDevice
    device_tags: Tags[AvdDeviceTag]
    interface_tags: Tags[AvdInterfaceTag]
    result: DeviceResult
    in_cv_inventory: bool = False
    in_cv_studios: bool = False
    serial_number: str | None = None
...
@dataclass
class Tags(Generic[T_AvdTag]):
    """Tags to be added or removed to match deployment input after reviewing existing tags within CV."""

    to_add: list[T_AvdTag] = field(default_factory=list)
    to_remove: list[T_AvdTag] = field(default_factory=list)


@dataclass
class TagsResult(Generic[T_AvdTag]):
    """Tags that were added, removed, verified, skipped as part of deployment to CV."""

    added: list[T_AvdTag] = field(default_factory=list)
    removed: list[T_AvdTag] = field(default_factory=list)
    verified: list[T_AvdTag] = field(default_factory=list)
    skipped: list[T_AvdTag] = field(default_factory=list)


@dataclass
class MiscComponentResult(Generic[T_AvdMiscComponent]):
    """Miscellaneous types such as AvdEosConfig, AvdPathfinderMetadata that were added, skipped as part of deployment to CV."""

    added: list[T_AvdMiscComponent] = field(default_factory=list)
    skipped: list[T_AvdMiscComponent] = field(default_factory=list)


class DeviceInventoryResult(Enum):
    ADDED = 1
    VERIFIED = 2
    REMOVED = 3
    MISSING = 4
...
class DeviceResult:
    ...
    inventory: DeviceInventoryResult
...
@dataclass
class AvdDuplicatedDevices:
    system_mac_address: AvdDuplicatedSystemMacAddress = field(default_factory=AvdDuplicatedSystemMacAddress)
    """Object holding AvdDevice objects with duplicated system_mac_address."""
    serial_number: dict[str, list[AvdDevice]] = field(default_factory=dict)
    """Dictionary holding AvdDevice objects with duplicated serial_number."""
