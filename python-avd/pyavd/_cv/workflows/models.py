...
class AvdManifest:
    ...
    scope_key: str = ""
    """
    Optional manifest scope key used to isolate cleanup between multiple independent manifest deployments
    sharing the same CloudVision instance (e.g. parallel CI jobs).

    When set, container and configlet IDs are generated within this scope and cleanup is restricted to
    entities belonging to this scope only. Different manifests must use different scope keys to avoid
    interfering with each other.

    When empty (default), the global AVD namespace is used and cleanup applies to all AVD-managed entities,
    ...
    def from_dict(cls, data: dict[str, Any]) -> AvdManifest:
        ...
        try:
            ...
            return cls(configlets=configlets, containers=containers, scope_key=scope_key)
...
class CVManifest:
    ...
    manifest_scope_prefix: str = AVD_ENTITY_PREFIX
    """
    Prefix used to identify entities belonging to this manifest during cleanup.

    When a scope_key is provided in the source AvdManifest, this is set to
    ``avd_{scope_id}_`` (where ``scope_id`` is the first 8 hex chars of
    ``uuid5(AVD_NAMESPACE, scope_key)``), restricting cleanup to only entities
    generated by this manifest scope.

    Defaults to ``avd_`` for backward-compatible global cleanup behavior.
    """

    @classmethod
    def from_avd_manifest(cls, avd_manifest: AvdManifest) -> CVManifest:
        """Build the desired CVManifest from the AVD input manifest."""
        # Derive the manifest namespace and scope prefix from scope_key.
        if avd_manifest.scope_key:
            scope_uuid = uuid5(AVD_NAMESPACE, avd_manifest.scope_key)
            manifest_ns = scope_uuid
            manifest_scope_prefix = f"{AVD_ENTITY_PREFIX}{scope_uuid.hex[:8]}_"
        else:
            manifest_ns = AVD_NAMESPACE
            manifest_scope_prefix = AVD_ENTITY_PREFIX
        ...
        for avd_configlet in avd_manifest.configlets:
            cv_configlet = CVConfiglet(
                ...
                id=cls._generate_deterministic_id(avd_configlet.name, namespace=manifest_ns, prefix=manifest_scope_prefix),
        ...
        for root_container in avd_manifest.containers:
            cls._process_container_recursively(
                container=root_container,
                parent_path="",
                cv_configlet_map=cv_configlet_map,
                cv_container_map=cv_container_map,
                manifest_ns=manifest_ns,
                manifest_scope_prefix=manifest_scope_prefix,
            )

        # Return the completed manifest.
        return cls(configlets=tuple(cv_configlet_map.values()), containers=tuple(cv_container_map.values()), manifest_scope_prefix=manifest_scope_prefix)
    ...
    def _process_container_recursively(
        ...
        child_ids = [
            cls._process_container_recursively(sub_container, current_path, cv_configlet_map, cv_container_map, manifest_ns, manifest_scope_prefix)
        ...
        cv_container = CVContainer(
            ...
            id=cls._generate_deterministic_id(current_path, namespace=manifest_ns, prefix=manifest_scope_prefix),
            is_root=(parent_path == ""),
    ...
    @staticmethod
    def _generate_deterministic_id(key: str, namespace: UUID = AVD_NAMESPACE, prefix: str = AVD_ENTITY_PREFIX) -> str:
        """Generate a deterministic ID from the provided namespace and key."""
        return f"{prefix}{uuid5(namespace, key)}"
