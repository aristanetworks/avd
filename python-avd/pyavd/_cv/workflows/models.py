...
from .input_models import AvdDeviceTag, AvdEosConfig, AvdInterfaceTag, AvdPathfinderMetadata
...
class DeployToCvResult:
    ...
    def update_from_workflow_devices(self, workflow_devices: list[WorkflowDevice]) -> None:
        # iterate thru each workflow device and update deployed/skipped/removed placeholder
        # for device tags/interface tags/config/pathfinder metadata in overall deployment result
        for device in workflow_devices:
            # iterate thru all device_tags checking for deployment status and accumulate under correct bucket
            for device_tag in device.device_tags:
                match device_tag.status:
                    case DeploymentStatus.SKIPPED:
                        if isinstance(cv_tag := device.get_one_cv_tag(device_tag.input), CVDeviceTag):
                            self.skipped_device_tags.append(cv_tag)
                    case DeploymentStatus.DEPLOYED:
                        if isinstance(cv_tag := device.get_one_cv_tag(device_tag.input), CVDeviceTag):
                            self.deployed_device_tags.append(cv_tag)
                    case DeploymentStatus.REMOVED:
                        if isinstance(cv_tag := device.get_one_cv_tag(device_tag.input), CVDeviceTag):
                            self.removed_device_tags.append(cv_tag)
                    case _:
                        # ideally we don't find anything here
                        pass
            # iterate thru all interface_tags checking for deployment status and accumulate under correct bucket
            for interface_tag in device.interface_tags:
                match interface_tag.status:
                    case DeploymentStatus.SKIPPED:
                        if isinstance(cv_tag := device.get_one_cv_tag(interface_tag.input), CVInterfaceTag):
                            self.skipped_interface_tags.append(cv_tag)
                    case DeploymentStatus.DEPLOYED:
                        if isinstance(cv_tag := device.get_one_cv_tag(interface_tag.input), CVInterfaceTag):
                            self.deployed_interface_tags.append(cv_tag)
                    case DeploymentStatus.REMOVED:
                        if isinstance(cv_tag := device.get_one_cv_tag(interface_tag.input), CVInterfaceTag):
                            self.removed_interface_tags.append(cv_tag)
                    case _:
                        # ideally we don't find anything here
                        pass
            # iterate thru config checking for deployment status and accumulate under correct bucket
            if device.config is not None:
                match device.config.status:
                    case DeploymentStatus.SKIPPED:
                        self.skipped_configs.append(device.get_cv_eos_config(device.config.input))
                    case DeploymentStatus.DEPLOYED:
                        self.deployed_configs.append(device.get_cv_eos_config(device.config.input))
                    case _:
                        # ideally we don't find anything here
                        pass
            # iterate thru pathfinder_metadata checking for deployment status and accumulate under correct bucket
            if device.pathfinder_metadata is not None:
                match device.pathfinder_metadata.status:
                    case DeploymentStatus.SKIPPED:
                        self.skipped_cv_pathfinder_metadata.append(device.get_cv_pathfinder_metadata(device.pathfinder_metadata.input))
                    case DeploymentStatus.DEPLOYED:
                        self.deployed_cv_pathfinder_metadata.append(device.get_cv_pathfinder_metadata(device.pathfinder_metadata.input))
                    case _:
                        # ideally we don't find anything here
                        pass
...
class AvdDevice:
    """Input generated by AVD for a particular device to be deployed."""
    ...
    device_tags: tuple[AvdDeviceTag, ...] = field(default_factory=tuple)
    interface_tags: tuple[AvdInterfaceTag, ...] = field(default_factory=tuple)
    pathfinder_metadata: AvdPathfinderMetadata | None = None

    @classmethod
    def from_device_input(
        cls,
        cv_device: CVDevice,
        config: AvdEosConfig | None,
        device_tags: tuple[AvdDeviceTag, ...],
        interface_tags: tuple[AvdInterfaceTag, ...],
        pathfinder_metadata: AvdPathfinderMetadata | None,
    ) -> AvdDevice:
        # used primarily for testing
        return cls(
            hostname=cv_device.hostname,
            serial_number=cv_device.serial_number,
            system_mac_address=cv_device.system_mac_address,
            config=config,
...
class WorkflowConfig:
    input: AvdEosConfig
    parent_device: WorkflowDevice
    status: DeploymentStatus = DeploymentStatus.PENDING


@dataclass
class WorkflowPathfinderMetadata:
    input: AvdPathfinderMetadata
    parent_device: WorkflowDevice
    status: DeploymentStatus = DeploymentStatus.PENDING
...
@dataclass
class WorkflowTag:
    input: AvdDeviceTag | AvdInterfaceTag
    parent_device: WorkflowDevice
    status: DeploymentStatus = DeploymentStatus.PENDING
    ...
    def as_cv_tag(self) -> CVTag:
        """Return the CVTag model for this tag."""
        return CVTag(
            element_type="device" if isinstance(self.input, AvdDeviceTag) else "interface",
    ...
    def as_cv_tag_assignment(self) -> CVTagAssignment | None:
        """Return the CVTagAssignment model for this tag."""
        if self.parent_device is None or self.parent_device.serial_number is None:
            return None
        ...
        return CVTagAssignment(
            ...
            label=self.input.label,
            value=self.input.value,
            device_id=self.parent_device.serial_number,
            interface_id=self.input.interface if isinstance(self.input, AvdInterfaceTag) else None,
        )


@dataclass
class WorkflowDevice:
    """Internal mutable representation of a device for deployment workflow based on inputs from AVD."""

    input: AvdDevice

    # State discovered from CloudVision
    in_cv_inventory: bool = False
    serial_number: str | None = None
    ...
    # Containers for stateful workflow objects
    config: WorkflowConfig | None = None
    device_tags: list[WorkflowTag] = field(default_factory=list)
    interface_tags: list[WorkflowTag] = field(default_factory=list)
    pathfinder_metadata: WorkflowPathfinderMetadata | None = None

    @classmethod
    def from_avd_input(cls, avd_input: AvdDevice) -> WorkflowDevice:
        # Build WorkflowDevice object from AvdDevice as input.
        workflow_device = WorkflowDevice(input=avd_input)
        # Build Workflow<elem> for <elem> found to be existing within AvdDevice input.
        if avd_input.config:
            workflow_device.config = WorkflowConfig(input=avd_input.config, parent_device=workflow_device)
        if avd_input.pathfinder_metadata:
            workflow_device.pathfinder_metadata = WorkflowPathfinderMetadata(input=avd_input.pathfinder_metadata, parent_device=workflow_device)
        if isinstance(avd_input.device_tags, tuple) and len(avd_input.device_tags) > 0:
            for device_tag in avd_input.device_tags:
                workflow_device.device_tags.append(WorkflowTag(input=device_tag, parent_device=workflow_device))
        if isinstance(avd_input.interface_tags, tuple) and len(avd_input.interface_tags) > 0:
            for interface_tag in avd_input.interface_tags:
                workflow_device.interface_tags.append(WorkflowTag(input=interface_tag, parent_device=workflow_device))
        return workflow_device
    ...
    def get_cv_device(self) -> CVDevice:
        ...
        return CVDevice(
            ...
            _exists_on_cv=self.in_cv_inventory,
            _streaming=self.streaming,
        )

    def get_cv_eos_config(self, config_object: AvdEosConfig) -> CVEosConfig:
        return CVEosConfig(file=config_object.file, device=self.get_cv_device(), configlet_name=config_object.configlet_name)
    ...
    def get_cv_pathfinder_metadata(self, pathfinder_metadata: AvdPathfinderMetadata) -> CVPathfinderMetadata:
        return CVPathfinderMetadata(metadata=pathfinder_metadata.metadata, device=self.get_cv_device())
