...
CHANGE_CONTROL_STATUS_TO_FINAL_STATE_MAP: dict[ChangeControlStatus, Literal["running", "completed", "scheduled"] | None] = {
...
async def finalize_change_control_on_cv(change_control_id: str, change_control: CVChangeControl, cv_client: CVClient) -> None:
    """
    ...
    cv_change_control = await cv_client.get_change_control(change_control_id=change_control_id)
    ...
    if change_control.name is None:
        change_control.name = cv_change.name
    if change_control.description is None:
        change_control.description = cv_change.notes

    # TODO: Add CC template

    # Update the change control with name, description etc from our local object if needed.
    if change_control.name != cv_change.name or change_control.description != cv_change.notes:
        await cv_client.set_change_control(change_control_id=change_control_id, name=change_control.name, description=change_control.description)
    ...
    await cv_client.start_change_control(change_control_id=change_control_id, description="Automatically started by AVD")
    ...
    cv_change_control = await cv_client.wait_for_change_control_state(cc_id=change_control_id, state="completed")
    if cv_change_control.error is not None:
