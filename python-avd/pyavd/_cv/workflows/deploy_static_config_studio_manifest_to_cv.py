...
async def _sync_configlets(cv_manifest: CVManifest, deployment_result: DeployToCvResult, cv_client: CVClient) -> None:
    ...
    # In "flexible" mode, don't delete any configlets.
    if cv_manifest.configlet_policy == "flexible":
        LOGGER.info("deploy_static_config_studio_manifest_to_cv: No configlet deletions when configlet_policy is set to flexible.")
        return

    # In "controlled" mode, delete AVD-managed configlets not declared in this manifest push. Manual configlets are preserved.
    ...
    if configlets_to_delete:
        LOGGER.info(
            "deploy_static_config_studio_manifest_to_cv: Deleting %d AVD-managed configlets not declared in this manifest push.", len(configlets_to_delete)
        )
