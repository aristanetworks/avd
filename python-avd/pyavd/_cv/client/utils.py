...
def remove_item_from_list(itm: Any, lst: list, matcher: Callable) -> None:
    """
...
def upsert_item_in_list(itm: Any, lst: list, matcher: Callable) -> None:
    """
...
def set_value_from_path(path: Sequence[str | int], data: list | dict, value: Any) -> None:
    """
    ...
    # Convert '0' to 0.
    path = [int(element) if isinstance(element, str) and element.isnumeric() else element for element in path]
    if len(path) == 1:
    ...
    if isinstance(data, dict):
        ...
        if isinstance(path[1], int):
            ...
            set_value_from_path(path[1:], data[path[0]], value)
        else:
            data.setdefault(path[0], {})
            set_value_from_path(path[1:], data[path[0]], value)
    elif isinstance(data, list) and isinstance(path[0], int):
        ...
        if isinstance(path[1], int):
            if not isinstance(data[index], list):
                data[index] = []
            set_value_from_path(path[1:], data[index], value)
        else:
