...
class WorkspaceMixin(Protocol):
    ...
    async def create_workspace(
        ...
        return await client.set(request, metadata=self._metadata, timeout=timeout)
    ...
    async def wait_for_new_workspace_readiness(
        ...
        async for response in responses:
            if response.value.state == WorkspaceState.PENDING:
                return response.value
            LOGGER.debug(
                "wait_for_workspace_readiness: Got workspace update but it is not yet in PENDING state. Latest response: %s",
                response,
            )

        msg = f"wait_for_workspace_readiness: Timed out waiting for Workspace '{workspace_id}' to be in PENDING state."
        raise CVResourceNotFound(msg)
