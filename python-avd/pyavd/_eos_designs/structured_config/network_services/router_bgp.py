...
class RouterBgpMixin(Protocol):
    ...
    def _update_router_bgp_vrf_mlag_neighbor_cfg(
        ...
        if (
            self._exclude_mlag_ibgp_peering_from_redistribute(vrf, tenant)
            and not self.inputs.avd_design_future.only_configure_route_map_connected_to_bgp_vrfs_when_required
        ):
