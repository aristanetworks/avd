...
class EthernetInterfacesMixin(UtilsMixin):
        ...
        for index, connected_endpoint in enumerate(self._filtered_connected_endpoints):
            for adapter in connected_endpoint.adapters:
                for node_index, node_name in enumerate(adapter.switches):
                    ...
                    context = f"{connected_endpoint._type}[name={connected_endpoint.name}].adapters[{index}]"
    ...
    def _get_ethernet_interface_cfg(
        ...
        if adapter.port_channel and adapter.port_channel.mode:
            ...
            lacp_fallback_mode = adapter.port_channel.lacp_fallback.mode if adapter.port_channel.lacp_fallback else None
            if lacp_fallback_mode == "static":
                ...
                ethernet_interface = self._update_ethernet_interface_cfg(
                    profile._cast_as(EosDesigns._DynamicKeys.DynamicConnectedEndpointsKeys.ConnectedEndpointsKeysKeyItem.AdaptersItem),
                    ethernet_interface,
                    connected_endpoint,
                    context,
                )
