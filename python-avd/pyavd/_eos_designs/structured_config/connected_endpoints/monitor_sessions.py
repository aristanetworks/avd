...
from pyavd._utils import append_if_not_duplicate, groupby_obj, merge, strip_null_from_data
...
class MonitorSessionsMixin(UtilsMixin):
    ...
    def monitor_sessions(self: AvdStructuredConfigConnectedEndpoints) -> list | None:
        ...
        for session_name, session_configs in groupby_obj(self._monitor_session_configs, "name"):
            ...
            merged_settings: dict = merge({}, [session_config._as_dict() for session_config in session_configs_list], destructive_merge=False)
            monitor_session = {
                "name": session_name,
                "sources": [],
