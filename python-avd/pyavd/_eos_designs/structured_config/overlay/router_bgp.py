    ...
    def _set_bgp_listen_ranges(self: AvdStructuredConfigOverlayProtocol) -> None:
        ...
        for prefix in self.shared_utils.wan_listen_ranges:
            self.structured_config.router_bgp.listen_ranges.append_new(
                prefix=prefix, peer_group=self.inputs.bgp_peer_groups.wan_overlay_peers.name, remote_as=self.shared_utils.get_asn(self.shared_utils.bgp_as)
            )
    ...
    def _set_peer_groups(self: AvdStructuredConfigOverlayProtocol) -> None:
        ...
        if self.shared_utils.overlay_routing_protocol == "ebgp":
            ...
            if self.shared_utils.overlay_mpls is True:
                ...
                mpls_peer_group.remote_as = self.shared_utils.get_asn(self.shared_utils.bgp_as)
            ...
            if self.shared_utils.overlay_evpn_vxlan is True:
                ...
                evpn_overlay_peer_group.remote_as = self.shared_utils.get_asn(self.shared_utils.bgp_as)
            ...
            if self._is_mpls_server is True:
                ...
                rr_overlay_peer_group.remote_as = self.shared_utils.get_asn(self.shared_utils.bgp_as)
        ...
        if self.shared_utils.is_wan_router:
            ...
            wan_overlay_peer_group._update(
                remote_as=self.shared_utils.get_asn(self.shared_utils.bgp_as), ttl_maximum_hops=self.inputs.bgp_peer_groups.wan_overlay_peers.ttl_maximum_hops
            )
