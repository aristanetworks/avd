...
def get_avd_facts(
    all_inputs: dict[str, dict], pool_manager: PoolManager | None = None, device_uid_to_hostname_map: dict[str, str] | None = None
    ...
    Args:
        all_inputs: A dictionary where keys are device_uid (often the same as hostname) and values are dictionaries of input variables per device.
            ```python
            {
                "<device_uid1>": dict,
                "<device_uid2>": dict,
    ...
    Returns:
        Dictionary with various internal "facts" keyed by device_uid. The full dict must be given as argument to `pyavd.get_device_structured_config`.
    """
    ...
    return get_facts(all_inputs=all_input_classes, pool_manager=pool_manager, all_hostvars=all_inputs, device_uid_to_hostname_map=device_uid_to_hostname_map)
