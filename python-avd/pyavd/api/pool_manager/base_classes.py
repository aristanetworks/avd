...
T_ValueType = TypeVar("T_ValueType", int, IPv4Interface, IPv6Interface)
...
@overload
def value_as_json(value: int) -> int: ...


@overload
def value_as_json(value: IPv4Interface) -> str: ...


@overload
def value_as_json(value: IPv6Interface) -> str: ...


def value_as_json(value: T_ValueType) -> int | str:
    match value:
        case int():
            return value
        case IPv4Interface() | IPv6Interface():
            return str(value)
...
class Pool(Generic[T_ValueType]):
    ...
    def _is_value_available(self, value: T_ValueType) -> bool:
        if not isinstance(value, self.collection.value_type):
            msg = f"The requested value '{value}' is a {type(value)}. Expected a {self.collection.value_type}"
            raise TypeError(msg)
        existing_ids = {assignment.value for assignment in self.assignments.values()}
        return value not in existing_ids and self.collection.is_valid_value(value, self)
    ...
    def get_assignment(self, key: str, requested_value: T_ValueType | None = None) -> PoolAssignment[T_ValueType]:
        ...
        if key not in self.assignments:
            ...
            new_assignment_value = (
                requested_value if requested_value is not None and self._is_value_available(requested_value) else self.collection._next_available(self)
            )
    ...
    def assignments_as_dict(self) -> dict[str, int | str]:
        ...
        assignments_as_dict = {assignment.value: assignment for assignment in self.assignments.values()}
        return {assignments_as_dict[value].key: value_as_json(value) for value in natural_sort(assignments_as_dict)}

    @classmethod
    def load(cls, pool_key: Any, pool_assignments: Any, collection: PoolCollection[T_ValueType]) -> None:
        ...
        for assignment_key, assignment_value in pool_assignments.items():
            ...
            # Try loading into correct type
            try:
                value = collection.value_type(assignment_value)
            except ValueError as e:
                msg = f"Invalid type for assignment 'value' '{type(assignment_value)}'. Expected a {collection.value_type.__name__}."
                raise TypeError(msg) from e

            assignment = collection.assignment_cls(assignment_key, value)
            assignments[assignment.key] = assignment

        collection.add_pool(pool_key, assignments)
...
class PoolCollection(ABC, Generic[T_ValueType]):
    ...
    min_value: T_ValueType | None = None
    ...
    def __post_init__(self) -> None:
        ...
        try:
            self.load(file_data[self.pools_key])
        except (TypeError, KeyError, ValueError) as e:
            msg = f"An error occurred during parsing of '{self.pools_file}': {e.__class__.__name__}: {e}"
            raise TypeError(msg) from e

    def load(self, data: Any) -> None:
        """Load pools from file data."""
        if not isinstance(data, dict):
            msg = f"Invalid type '{type(data)}'. Expected a dict."
            raise TypeError(msg)

        for pool_key, pool_assignments in data.items():
            self.pool_cls.load(pool_key, pool_assignments, collection=self)
    ...
    @staticmethod
    @abstractmethod
    def is_valid_value(value: T_ValueType, pool: Pool[T_ValueType]) -> bool:
        """Check if a value is valid according to the pool definition."""

    @abstractmethod
    def add_pool(self, pool_key: str, assignments: dict[str, PoolAssignment[T_ValueType]] | None = None) -> None:
        """Creates a new pool and add it to the collection with the given assignments or as an empty pool."""

    @abstractmethod
    def _next_available(self, pool: Pool[T_ValueType]) -> T_ValueType:
