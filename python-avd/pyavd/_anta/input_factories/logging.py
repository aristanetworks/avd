...
class VerifyLoggingErrorsInputFactory(AntaTestInputFactory[VerifyLoggingErrors.Input]):
    ...
    Generates inputs using `validate_no_errors_period` from metadata to check the last N days if configured.
    ...
    def create(self) -> list[VerifyLoggingErrors.Input] | None:
        ...
        return [VerifyLoggingErrors.Input(last_number_time_units=last_number_time_units, time_unit="days")]
