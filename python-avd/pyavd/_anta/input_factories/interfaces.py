...
from anta.tests.interfaces import VerifyInterfacesStatus, VerifyL2MTU, VerifyL3MTU, VerifyPortChannels
...
class VerifyL3MTUInputFactory(AntaTestInputFactory):
    ...
    def create(self) -> list[VerifyL3MTU.Input] | None:
        ...
        interfaces, ignored_interfaces = self.device.l3_mtu_interfaces
...
class VerifyL2MTUInputFactory(AntaTestInputFactory):
    """Input factory class for the `VerifyL2MTU` test."""

    def create(self) -> list[VerifyL2MTU.Input] | None:
        """Create a list of inputs for the `VerifyL2MTU` test."""
        interfaces, ignored_interfaces = self.device.l2_mtu_interfaces

        return [VerifyL2MTU.Input(ignored_interfaces=natural_sort(ignored_interfaces), specific_mtu=natural_sort(interfaces))] if interfaces else None
