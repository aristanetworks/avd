...
def create_catalog(
    ...
    for test in test_specs:
        for test_def in create_test_definitions(test, device_context):
...
def create_test_definitions(test_spec: TestSpec, device_context: DeviceTestContext) -> Iterator[AntaTestDefinition]:
    """Generate the AntaTestDefinition's from this TestSpec instance."""
    ...
    if test_spec.input_factory is not None:
        ...
        # If no inputs were found AND the factory wasn't explicitly skipped by a decorator, log a message
        if not has_inputs and not factory._skipped_by_decorator:
