...
@pytest.mark.parametrize(
    "run_name",
    ["default_run", "allow_bgp_vrfs_run", "filtered_run"],
...
def test_get_device_test_catalog(molecule_host: MoleculeHost, molecule_scenario: MoleculeScenario, run_name: RunName) -> None:
    ...
    if run_name == "allow_bgp_vrfs_run":
        settings = SETTINGS_WITH_BGP_VRFS
    elif run_name == "filtered_run":
        settings = get(TEST_CASES[run_name], f"{molecule_host.name}", SETTINGS_FILTERED_DEFAULT)
