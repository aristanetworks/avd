...
def test_eos_designs_validate_dict_with_valid_data(hostname: str, all_inputs: dict) -> None:
    """Test EosDesigns model with valid data."""
    inputs = all_inputs[hostname]

    with warnings.catch_warnings():
        warnings.simplefilter("error")
        assert EosDesigns._validate_dict(inputs)
...
def test_eos_cli_config_gen_validate_dict_with_valid_data(hostname: str, structured_configs: dict) -> None:
    """
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        assert EosCliConfigGen._validate_dict(structured_config)
